...
public class UploadHandler {
    ...
    private ArrayList<String> mAuthority = null;
    private static final String BROWSER_NEEDED_AUTHORITY = "user-assigned-authorities";

    public UploadHandler(Controller controller) {
        mController = controller;
        ActivityManager.MemoryInfo memInfo = new ActivityManager.MemoryInfo();
        Context context = mController.getContext();
        ((ActivityManager) context.getSystemService(Activity.ACTIVITY_SERVICE))
                .getMemoryInfo(memInfo);
        if ((memInfo.totalMem / 1024.0 / 1024.0) < 512.0) {
            mIsLowMemory = true;
            mAuthority = new ArrayList<String>();
            mAuthority.add("com.android.externalstorage.documents");
            mAuthority.add("com.android.providers.downloads.documents");
        } else {
            mIsLowMemory = false;
        }
    }
    ...
    void openFileChooser(ValueCallback<Uri[]> callback, FileChooserParams fileChooserParams) {
        ...
        if (mIsLowMemory) {
            ...
            intent.putStringArrayListExtra(BROWSER_NEEDED_AUTHORITY, mAuthority);
