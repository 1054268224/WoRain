...
public class Controller
    ...
    private static final String SAVE_PAGE_DIR
                         = File.separator + "Download" + File.separator + "SavedPages";
    ...
    /// M: add for release tabs when memory is low
    public static final int MSG_RELEASE_TABS = 1100;
    ...
    /// M: Add for delay last tab to be removed @{
    private boolean mDelayRemoveLastTab = false;
    /// @}

    public Controller(Activity browser) throws Exception {
        mActivity = browser;
    ...
    private void onPreloginFinished(Bundle icicle, Intent intent, long currentTabId,
            boolean restoreIncognitoTabs) throws Exception {
        if (currentTabId == -1) {
            ...
            if (intent.getData() != null
                ...
                if (t != null) {
                    t.setAppId(intent.getStringExtra(com.android.browser.provider.Browser.EXTRA_APPLICATION_ID));
                }
                WebView webView = t.getWebView();
                if (extra != null) {
                    int scale = extra.getInt(com.android.browser.provider.Browser.INITIAL_ZOOM_LEVEL, 0);
...
/*
    ...
    @Override
    public void createSubWindow(Tab tab) throws Exception {
        endActionMode();
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    case UPDATE_BOOKMARK_THUMBNAIL:
                        ...
                        break;
                    /// @M add for release tabs when memory is low @ {
                    case MSG_RELEASE_TABS:
                        try {
                            getTabControl().freeMemory();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        // dump memory after release tabs
                        new CheckMemoryTask(mHandler)
                                .execute(getTabControl().getVisibleWebviewNums(), null,
                                         false, null, getTabControl().getFreeTabIndex(), false);
                        break;
                    /// @ }
                }
            }
     ...
     *          with {@link com.android.browser.provider.Browser#EXTRA_SHARE_FAVICON}
     * @param screenshot Bitmap of a screenshot of the page.  Stored in the
     *          Intent with {@link com.android.browser.provider.Browser#EXTRA_SHARE_SCREENSHOT}
     */
    static final void sharePage(Context c, String title, String url,
        ...
        send.putExtra(com.android.browser.provider.Browser.EXTRA_SHARE_FAVICON, favicon);
        send.putExtra(com.android.browser.provider.Browser.EXTRA_SHARE_SCREENSHOT, screenshot);
        try {
    ...
    @Override
    public void onConfgurationChanged(Configuration config) throws Exception {
        mConfigChanged = true;
    ...
    @Override
    public void handleNewIntent(Intent intent) throws Exception {
    ...
    public void onPause() {
        ...
        if (tab != null) {
            tab.pause();
            /// M: Add for delay last tab to be removed
            if (!mDelayRemoveLastTab && !pauseWebViewTimers(tab)) {
                if (mWakeLock == null) {
    ...
    private void resumeWebViewTimers(Tab tab) {
        ...
        if ((!mActivityPaused && !inLoad) || (mActivityPaused && inLoad)) {
            ...
            WebViewTimersControl.getInstance().onBrowserActivityResume(w, this);
        }
    }
    ...
    @Override
    public void onPageStarted(Tab tab, WebView view, Bitmap favicon) throws Exception {
        ...
        if (tab.inForeground()) {
            mUi.updateBottomBarState(true, tab.canGoBack() || tab.getParent() != null,
                                     tab.canGoForward());
        }
    ...
    @Override
    public void onPageFinished(Tab tab) throws Exception {

        if(!mDelayRemoveLastTab) {
            if (DEBUG) {
                Log.i(LOGTAG, "onPageFinished backupState " + tab.getUrl());
            }
        ...
        if (tab.getWebView() != null && tab.inForeground()) {
            ...
            mUi.updateBottomBarState(pageCanScroll, tab.canGoBack() || tab.getParent() != null,
                                     tab.canGoForward());
        }
        // Performance probe
        if (false) {
        ...
        Performance.dumpSystemMemInfo(mActivity);
        ArrayList<Integer> currentTabIndex = new ArrayList<Integer>();
        currentTabIndex.add(getTabControl().getTabPosition(tab));
        new CheckMemoryTask(mHandler).execute(getTabControl().getVisibleWebviewNums(),
                currentTabIndex, true, tab.getUrl(), null, false);
        /// @ }
    }
    ...
    public void onProgressChanged(Tab tab) {
        ...
        if (DEBUG) {
            Log.i(LOGTAG, "Network_Issue onProgressChanged url: " + tab.getUrl()
                    + " : " + newProgress + "%");
        }
...
//        }
    ...
    @Override
    public void onUpdatedSecurityState(Tab tab) throws Exception {
        mUi.onTabDataChanged(tab);
    }

    @Override
    public void onReceivedTitle(Tab tab, final String title) throws Exception {
        mUi.onTabDataChanged(tab);
    ...
    @Override
    public void onFavicon(Tab tab, WebView view, Bitmap icon) throws Exception {
        mUi.onTabDataChanged(tab);
        maybeUpdateFavicon(tab, view.getOriginalUrl(), view.getUrl(), icon);
    }

    @Override
    public boolean shouldOverrideUrlLoading(Tab tab, WebView view, String url) throws Exception {
    ...
    public void onDownloadStart(Tab tab, String url, String userAgent,
        ...
        if (DEBUG) {
            Log.d(XLOGTAG, "onDownloadStart: dispos=" +
                                    ...
                                    public void onClick(DialogInterface dialog,
                                        ...
                                        try {
                                            DownloadHandler.onDownloadStartNoStream(activity,
                                                downloadUrl, downloadUserAgent,
                                                downloadContentDisposition, downloadMimetype,
                                                null, false, downloadContentLength);
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
                                        ...
                                        if (web != null &&
                                            ...
                                            // This Tab was opened for the sole purpose
                                            // of downloading a file. Remove it.
                                            if (downloadTab == downloadTabControl
                                                                   .getCurrentTab()) {
                                                // In this case, the Tab is still on top.
                                                try {
                                                    goBackOnePageOrQuit();
                        ...
                        .setNegativeButton(R.string.open_content,
                                new DialogInterface.OnClickListener() {
                                    public void onClick(DialogInterface dialog,
                                            int whichButton) {
                                        if (downloadUrl != null) {
                                            String urlCookie = CookieManager.getInstance()
                                                                        .getCookie(downloadUrl);
                                            if (DEBUG) {
                                                Log.i(XLOGTAG, "url: " + downloadUrl +
                                                     " url cookie: " + urlCookie);
                                            }
                                            if (urlCookie != null) {
                                        ...
                                        Log.d(XLOGTAG,
                                              "User decide to open the content by startActivity");
                                        WebView web = downloadTab.getWebView();
                                        if (web != null &&
                                            web.copyBackForwardList().getSize() == 0) {
                                            // This Tab was opened for the sole purpose
        ...
        if (DEBUG) {
            Log.d(XLOGTAG, "onDownloadStart: download directly, mimetype="
                 + mimetype + ", url=" + url);
        }
        downloadStart(mActivity, url, userAgent, contentDisposition, mimetype, referer, false,
                contentLength, w, tab);
    }

    /// M: Fix 02827397: fix dynamic permission issue @{
    private void downloadStart(final Activity activity, final String url, final String userAgent,
            final String contentDisposition, final String mimetype, final String referer,
            final boolean privateBrowsing, final long contentLength, final WebView w,
            final Tab tab) throws Exception {

        final List<String> ungranted = PermissionHelper.getInstance().getUngrantedPermissions(
                STORAGE_PERMISSIONS);
        if (ungranted.size() == 0) {

            DownloadHandler.onDownloadStart(mActivity, url, userAgent, contentDisposition,
                    mimetype, referer, false, contentLength);
            if (w != null && w.copyBackForwardList().getSize() == 0) {
                // This Tab was opened for the sole purpose of downloading a
                // file. Remove it.
                if (tab == mTabControl.getCurrentTab()) {
                    ///M: Fix ALPS03114804: fix download tab does not close issue @{
                    // In Android N, AMS will not add FLAG_ACTIVITY_BROUGHT_TO_FRONT
                    // flag automaticly, so the following code is not effective
            ...
            PermissionHelper.getInstance().requestPermissions(ungranted,
                    new PermissionHelper.PermissionCallback() {
                        public void onPermissionsResult(int requestCode, String[] permissions,
                                int[] grantResults) throws Exception {
                            Log.d(XLOGTAG, " onRequestPermissionsResult "
                                    + requestCode);
                            if (grantResults != null && grantResults.length > 0) {
                                ...
                                for (String p : ungranted) {
                                    ...
                                    for (int i = 0; i < grantResults.length; i++) {
                                        if (p.equalsIgnoreCase(permissions[i])
                                                && grantResults[i] ==
                                    ...
                                    if (!match) {
                                        Log.d(XLOGTAG, p + " is not granted !");
                                        isGranted = false;
                                        break;
                                    }
                                }
                                if (isGranted) {
                                    DownloadHandler.onDownloadStart(mActivity, url, userAgent,
                                            contentDisposition, mimetype, referer, false,
                                            contentLength);
                                }
                                if (w != null && w.copyBackForwardList().getSize() == 0) {
                                    // This Tab was opened for the sole purpose of downloading a
            ...
            case COMBO_VIEW:
                ...
                if (Intent.ACTION_VIEW.equals(intent.getAction())) {
                    ...
                    if (id >= 0) {
                        Tab t = getCurrentTab();
                        t.mSavePageUrl = url;
                        t.mSavePageTitle = intent
                                           .getStringExtra(ComboViewActivity.EXTRA_OPEN_TITLE);
                        loadUrl(t, url);
                    }
                }
                break;
    ...
    // key handling
    public void onBackKey() throws Exception {
        if (!mUi.onBackKey()) {
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        if (DEBUG) {
            Log.d(XLOGTAG, "onCreateContextMenu type is : " + type);
        }
        ...
        if (v instanceof BrowserWebView) {
            urlExtra = ((BrowserWebView)webview).getSiteNavHitURL();
        }
        final String imageAnchorUrlExtra = urlExtra;
        if (DEBUG) {
            Log.d(XLOGTAG, "sitenavigation onCreateContextMenu imageAnchorUrlExtra is : "
                 + imageAnchorUrlExtra);
        }
        ...
        TelephonyManager telephony = (TelephonyManager) this.getContext()
        ...
        if (BrowserFeatureOption.BROWSER_SITE_NAVIGATION_SUPPORT) {
            ...
            if (url != null && url.equalsIgnoreCase(SiteNavigation.SITE_NAVIGATION)) {
                ...
                if (itemUrl != null) {
                    ...
                    Log.d(XLOGTAG,
                        "sitenavigation onCreateContextMenu itemUrl is null!");
                    menu.setGroupVisible(R.id.SITE_NAVIGATION_ADD, false);
                    menu.setGroupVisible(R.id.SITE_NAVIGATION_EDIT, false);
                    menu.setGroupVisible(R.id.PHONE_MENU, false);
                    menu.setGroupVisible(R.id.NO_PHONE_MENU, false);
    ...
    private void deleteSiteNavigationItem(final String itemUrl) {
        ...
        try {
            ...
            if (null != cursor && cursor.moveToFirst()) {
                Uri uri = ContentUris.withAppendedId(SiteNavigation.SITE_NAVIGATION_URI,
                                                     cursor.getLong(0));
    ...
    private String getNameFromUrl(String itemUrl) {
        ...
        if (DEBUG) {
            Log.d(XLOGTAG, "sitenavigation getNameFromUrl url is : " + itemUrl
    ...
    private void updateSiteNavigationThumbnail(final String itemUrl,
        ...
        int width = mActivity.getResources()
                    .getDimensionPixelOffset(R.dimen.siteNavigationThumbnailWidth);
        int height = mActivity.getResources()
                    .getDimensionPixelOffset(R.dimen.siteNavigationThumbnailHeight);
...
//        }
        ...
        /// M: When full screen or quick control can be use,
        /// hide this forward key. user can use bottom bar @{
        if (BrowserSettings.getInstance().useFullscreen()
            || BrowserSettings.getInstance().useQuickControls()) {
    ...
    @Override
    public boolean onOptionsItemSelected(MenuItem item) throws Exception {
...
//                break;
    ...
    public void toggleUserAgent() {
        ...
        if (mSettings.hasDesktopUseragent(web)) {
            ...
            header.put(com.android.browser.Browser.HEADER, com.android.browser.Browser.UAPROF);
            web.loadUrl(web.getOriginalUrl(), header);
        }
    }
    ...
    private void goLive() throws Exception {
        Tab t = getCurrentTab();
...
//                    mCrashRecoveryHandler.clearState();
                    File state = new File(getActivity().getApplicationContext().getCacheDir(),
                                          STATE_FILE);
                    if (state.exists()) {
                        state.delete();
                    }
    ...
    @Override
    public boolean onContextItemSelected(MenuItem item) throws Exception {
    ...
    static Bitmap createScreenshot(WebView view, int width, int height) {
        ...
        /// M: Fix 2807557: After drag down the page, its thumnail may be blank.
        /// That is because we always draw the begging of the web page.
        /// Need to relocate the canvas to the visible region. @{
        final int left = view.getScrollX();
        final int top = view.getScrollY() + view.getVisibleTitleHeight();
        canvas.translate(-left, -top);
        if (DEBUG) {
            Log.d(TAG, "createScreenShot()--->left = "
                + left + ", top = " + top + ", overviewScale = " + overviewScale);
        }
    ...
    private boolean needToIgnore(String orignalUrl, String url) {
        return false;
    }
    ...
    private void updateScreenshot(Tab tab) {
        ...
        if (!Patterns.WEB_URL.matcher(url).matches() && !tab.isBookmarkedSite()
                && !isDefaultBookmarks(originalUrl)) {
            return;
        }

        if (needToIgnore(originalUrl, url)) {
            return;
        }

        if (url != null && Patterns.WEB_URL.matcher(url).matches()) {
            String urlHost = (new WebAddress(url)).getHost();
            /// M: fix CR:414688, when run Bbench tool,
            /// update bookmark url: file:///data/bbench/index.html
...
*/
    ...
    protected void removeTab(Tab tab) throws Exception {
        mUi.removeTab(tab);
    ...
    @Override
    public void setActiveTab(Tab tab) throws Exception {
        ...
        if (tab != null) {
            ...
            if (mSettings.isDesktopUserAgent(wv)) {
                  mSettings.changeUserAgent(wv, false);
            }
        }
        if (DEBUG) {
    ...
    protected void closeEmptyTab() throws Exception {
        Tab current = mTabControl.getCurrentTab();
        if (current != null
    ...
    protected void reuseTab(Tab appTab, IntentHandler.UrlData urlData) throws Exception {
    ...
    public void removeSubWindow(Tab t) {
        if (t.getSubWebView() != null) {
            /// M: request focus before remove subview, if not, UrlInputView will get focus. @{
            WebView wv = t.getWebView();
            if (wv != null) {
                wv.requestFocus();
            }
            /// @}
    ...
    private Tab showPreloadedTab(final IntentHandler.UrlData urlData) throws Exception {
        if (DEBUG) {
    ...
    public Tab openTab(IntentHandler.UrlData urlData) throws Exception {
    ...
    @Override
    public Tab openTabToHomePage() throws Exception {
        if (DEBUG) {
    ...
    @Override
    public Tab openIncognitoTab() throws Exception {
    ...
    public Tab openTab(String url, boolean incognito, boolean setActive,
            boolean useCurrent, Tab parent) throws Exception {
        if (DEBUG) {
     ...
     */
    @Override
    public boolean switchToTab(Tab tab) throws Exception {
        if (DEBUG) {
    ...
    protected boolean closeCurrentTab(boolean andQuit) throws Exception {
        if (DEBUG) {
     ...
     */
    @Override
    public void closeTab(Tab tab) throws Exception {
        if (DEBUG) {
            Log.i(TAG, "Controller.closeTab()--->tab is " + tab);
         }
        /// M: add for browser memory optimization
        ArrayList<Integer> closeTabIndex = new ArrayList<Integer>();
        closeTabIndex.add(mTabControl.getTabPosition(tab));
        /// @ }
        if (tab == mTabControl.getCurrentTab()) {
            closeCurrentTab();
        } else {
        ...
        /// M: add for browser memory optimization, dump memory after close tab @ {
        new CheckMemoryTask(mHandler).execute(getTabControl().getVisibleWebviewNums(),
                closeTabIndex, false, null, null, true);
        /// @ }
    }
    ...
    public void closeOtherTabs() throws Exception {
        ...
        ArrayList<Integer> closeTabIndexs = new ArrayList<Integer>();
        for (int i = inactiveTabs; i >= 0; i--) {
            Tab tab = mTabControl.getTab(i);
            if (tab != mTabControl.getCurrentTab()) {
                /// M: add for browser memory optimization, keep the close tab index
                int tabIndex = mTabControl.getTabPosition(tab);
                closeTabIndexs.add(tabIndex);
                /// @ }
                removeTab(tab);
            }
        }
        /// M: add for browser memory optimization, dump memory after close tab @ {
        new CheckMemoryTask(mHandler).execute(getTabControl().getVisibleWebviewNums(),
                closeTabIndexs, false, null, null, true);
        /// @ }
    }

    // Called when loading from context menu or LOAD_URL message
    protected void loadUrlFromContext(String url) throws Exception {
        if (DEBUG) {
     ...
     */
    @Override
    public void loadUrl(Tab tab, String url) throws Exception {
        loadUrl(tab, url, null);
    }

    protected void loadUrl(Tab tab, String url, Map<String, String> headers) throws Exception {
        if (DEBUG) {
     ...
     */
    protected void loadUrlDataIn(Tab t, IntentHandler.UrlData data) throws Exception {
        if (DEBUG) {
    ...
    @Override
    public void onUserCanceledSsl(Tab tab) throws Exception {
    ...
    void goBackOnePageOrQuit() throws Exception {
        Tab current = mTabControl.getCurrentTab();
        if (current == null) {
            /*
        ...
        if (current.canGoBack()) {
            ...
            if (parent != null) {
                 ...
                 * moveTaskToBack().
                 */
                boolean result = mActivity.moveTaskToBack(true);
                Log.d(LOGTAG, "moveTaskToBack: " + result);
                if (result) {
                    onPause();
                }
            }
...
//          case KeyEvent.KEYCODE_Z:    // unused
    ...
    @Override
    public void onShowPopupWindowAttempt(final Tab tab,
            ...
            switch (m.what) {
                ...
                case UPDATE_SAVE_PAGE:
                    ...
                    Cursor queryUpdateTitle = mCr.query(Snapshots.CONTENT_URI, QUERY_TITLE,
                        Snapshots.JOB_ID + " = ? and " + Snapshots.IS_DONE + " = ?",
                        args, null);
                    ...
                    mBuilder.setContentTitle(title).setProgress(100, m.arg1, false)
                                                   .setContentInfo(m.arg1 + "%")
                ...
                case FINISH_SAVE_PAGE:
                    ...
                    String[] QUERY_PROJECTION =
                            new String[]{Snapshots.TITLE, Snapshots.VIEWSTATE_PATH, };
                    ...
                    while (c.moveToNext()) {
                        ...
                        /// M: Add to scan for the saved pages.
                        /// So that it can be opened in FileManager. @{
                    ...
                    builderFinish.setContentIntent(createSavePagePendingIntent())
                                 .setAutoCancel(true)
                                 .setOngoing(false)
                                 .setContentTitle(title)
                                 .setSmallIcon(R.drawable.ic_save_page_notification)
                                 .setContentText(mActivity.getText(R.string.saved_page_complete));
                ...
                case FAIL_SAVE_PAGE:
                    ...
                    if (title != null) {
                        builder
                            .setContentTitle(mActivity.getText(R.string.saved_page_fail) + title)
    ...
    class BrowserSavePageClient extends SavePageClient {
        ...
        public void getSaveDir(ValueCallback<String> callback, boolean canSaveAsComplete) {
            if (mTab != null) {
                ...
                Log.d(SAVE_PAGE_LOGTAG, "save dir:" + mSavePageFolder +
                      File.separator + subFolder.toString() + File.separator);
                callback.onReceiveValue(mSavePageFolder +
                                        File.separator +
                                        subFolder.toString() +
                                        File.separator);
            }
        ...
        public void onSaveProgressChange(int progress, int id) {
            ...
            int oriProgress = mProgress.get(id);
            if ((progress - oriProgress < 25) && progress != 100) {
                return;
            }
            // 100 maybe report many times.
            if (oriProgress == progress) {
                return;
            }
            mProgress.put(id, progress);
            Message updateSavePage = mSavePageHandler.obtainMessage(UPDATE_SAVE_PAGE,
                                                                    progress, id);
        ...
        public void onSaveFinish(int flag, int id) {
            ...
            switch (flag) {
                case 1:
                    Message finishSavePage = mSavePageHandler.obtainMessage(FINISH_SAVE_PAGE,
                                                                            id, 0);
                    finishSavePage.sendToTarget();
                    break;
                default:
                    Toast.makeText(mActivity, R.string.saved_page_failed, Toast.LENGTH_LONG)
                         .show();
    ...
    private PendingIntent createSavePagePendingIntent() {
        ...
        intent.putExtra(ComboViewActivity.EXTRA_INITIAL_VIEW,  UI.ComboViews.Snapshots.name());
    ...
    /// M: add for MTK bookmarks @ {
    private boolean isDefaultBookmarks(String originalUrl) {
        Resources res = mActivity.getResources();
        final CharSequence[] bookmarks = res.getTextArray(R.array.bookmarks);
        int size = bookmarks.length;
        for (int i = 0; i < size; i = i + 2) {
            if (originalUrl.equalsIgnoreCase(bookmarks[i + 1].toString())) {
                return true;
            }
        }
        return false;
