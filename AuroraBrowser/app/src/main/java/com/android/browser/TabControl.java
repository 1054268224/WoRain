...
class TabControl {
     ...
     *         number of open tabs.
     */
    Tab createNewTab(boolean privateBrowsing) throws Exception {
        return createNewTab(null, privateBrowsing);
    }

    Tab createNewTab(Bundle state, boolean privateBrowsing) throws Exception {
        int size = mTabs.size();
     ...
     */
    Tab createNewTab() throws Exception {
     ...
     */
    boolean removeTab(Tab t) throws Exception {
        if (t == null) {
            return false;
        }
    ...
    void destroy() throws Exception {
        Log.d(LOGTAG, "TabControl.destroy()--->Destroy all the tabs");
        for (Tab t : mTabs) {
            t.destroy();
        }
        mTabs.clear();
    ...
    void restoreState(Bundle inState, long currentId,
            boolean restoreIncognitoTabs, boolean restoreAll) throws Exception {
        if (currentId == -1) {
            return;
        }
     ...
     * WebView cache;
     */
    void freeMemory() throws Exception {
    ...
    private Vector<Tab> getHalfLeastUsedTabs(Tab current) {
        ...
        String optimize = android.os.SystemProperties.get("ro.vendor.gmo.ram_optimize");
        if (openTabCount > 2 && optimize != null && optimize.equals("1")) {
            openTabCount = (openTabCount+1) / 2;
        } else {
            openTabCount /= 2;
        }
     ...
     */
    void recreateWebView(Tab t) throws Exception {
     ...
     */
    private WebView createNewWebView() throws Exception {
     ...
     *        WebView.
     */
    private WebView createNewWebView(boolean privateBrowsing) throws Exception {
     ...
     *               set.
     */
    boolean setCurrentTab(Tab newTab) throws Exception {
     ...
     */
    private boolean setCurrentTab(Tab newTab, boolean force) throws Exception {
        ...
        boolean pageCanScroll = newTab.getWebView().canScrollVertically(-1)
                    || newTab.getWebView().canScrollVertically(1);
        mController.getUi().updateBottomBarState(pageCanScroll,
                                                 newTab.canGoBack() || newTab.getParent() != null,
                                                 newTab.canGoForward());
        return true;
    }
