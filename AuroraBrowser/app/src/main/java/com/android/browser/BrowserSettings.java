...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    public void startManagingSettings(WebSettings settings) throws Exception {
    ...
    public static String getFactoryResetUrlFromRes(Context context) throws Exception {
        /// M: Get the customer homepage. @{
     ...
     */
    private void syncSetting(WebSettings settings) throws Exception {
        if (DEBUG) {
        ...
        if (ua != null) {
            ...
            CustomProperties c = new CustomProperties();
            String operatorUA = CustomProperties.getString(c.getConst("MODULE_BROWSER"),c.getConst("USER_AGENT"));
    ...
    private void syncManagedSettings() throws Exception {
        if (DEBUG) {
    ...
    private void updateSearchEngine(boolean force) throws Exception {
    ...
    public void resetDefaultPreferences() throws Exception {
    ...
    private void resetCachedValues() throws Exception {
        updateSearchEngine(false);
    }
    ...
    public void changeUserAgent(WebView view, boolean isDesktop) throws Exception {
        if (view == null) {
            return;
        }
        WebSettings settings = view.getSettings();
    ...
    public String getSearchEngineName() throws Exception {

        SearchEngineManager searchEngineManager = new SearchEngineManager(mContext);
        List<com.wheatek.reflect.SearchEngine> searchEngines = searchEngineManager.getAvailables();
        if (searchEngines == null || searchEngines.size() <= 0) {
            return null;
        }
        String defaultSearchEngine = SearchEngine.GOOGLE;
        com.wheatek.reflect.SearchEngine searchEngine = searchEngineManager.getDefault();
        if (searchEngineManager.getDefault() != null) {
        ...
        if (searchEngine != null) {
            ...
            searchEngineFavicon = mPrefs.getString(PREF_SEARCH_ENGINE_FAVICON, "");
        }
        ...
        if (need_refresh && selectedItem != -1) {
            ...
            editor.putString(PREF_SEARCH_ENGINE, entryValues[selectedItem]);
            editor.putString(PREF_SEARCH_ENGINE_FAVICON,
    ...
    */
     ...
     * @return
     */
    public String getDefaultDownloadPathWithMultiSDcard() throws Exception {
        sBrowserSettingExt = Extensions.getSettingPlugin(mContext);
        if (DEBUG) {
    ...
    /// M: Add for Regional Phone support. @{
    public void updateSearchEngineSetting() throws Exception {
        ...
        SearchEngineManager searchEngineManager = new SearchEngineManager(mContext);
        com.wheatek.reflect.SearchEngine searchEngineInfo = searchEngineManager.getByName(searchEngineName);
        ...
        editor.putString(PREF_SEARCH_ENGINE_FAVICON, searchEngineFavicon);
        editor.commit();
