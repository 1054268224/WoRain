...
import com.wheatek.reflect.Search;
...
public class IntentHandler {
    ...
    void onNewIntent(Intent intent) throws Exception {
        ...
        if (BrowserActivity.ACTION_SHOW_BOOKMARKS.equals(action)) {
            mController.bookmarksOrHistoryPicker(UI.ComboViews.Bookmarks);
            return;
        }
        ...
        if (Intent.ACTION_VIEW.equals(action)
            ...
            if (intent.getBooleanExtra(com.android.browser.provider.Browser.EXTRA_CREATE_NEW_TAB, false)
                  || urlData.isPreloaded()) {
            ...
            final String appId = intent
                    .getStringExtra(com.android.browser.provider.Browser.EXTRA_APPLICATION_ID);
            if (DEBUG) {
    ...
    protected static UrlData getUrlDataFromIntent(Intent intent) throws Exception {
        String url = "";
            ...
            if (Intent.ACTION_VIEW.equals(action) ||
                ...
                if (url != null && url.startsWith("http")) {
                    final Bundle pairs = intent
                            .getBundleExtra(com.android.browser.provider.Browser.EXTRA_HEADERS);
    ...
    static boolean handleWebSearchIntent(Activity activity,
            Controller controller, Intent intent) throws Exception {
        if (DEBUG) {
