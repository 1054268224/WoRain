...
import com.wheatek.SyncStateContentProviderHelper;
...
public class BrowserProvider2 extends SQLiteContentProvider {
    ...
    public static final String LEGACY_AUTHORITY = "MtkBrowserProvider";
    ...
    private static final String TABLE_BOOKMARKS_JOIN_HISTORY =
        "history LEFT OUTER JOIN bookmarks ON " +
        "(history.url = bookmarks.url AND bookmarks.deleted=0 AND bookmarks.folder=0)";
    ...
    final class DatabaseHelper extends SQLiteOpenHelper {
        ...
        boolean importFromBrowserProvider(SQLiteDatabase db) {
            ...
            if (oldDbFile.exists()) {
                ...
                try {
                    ...
                    String tableFolder =
                        BrowserProvider.TABLE_NAMES[BrowserProvider.URI_MATCH_BOOKMARK_FOLDERS];
                    ...
                    if (c != null) {
                        while (c.moveToNext()) {
                            position++;
                            ContentValues values = new ContentValues();
                            //In MTK bookmark folder feature, root folder id is 0. However,
                            //it is 1 in google android4.0. So we have increase 1 to match them.
                            values.put(Bookmarks._ID,
                                       c.getInt(BOOKMARK_FOLDERS_PROJECTION_ID_INDEX) + 1);
                            values.put(Bookmarks.TITLE,
                                       c.getString(BOOKMARK_FOLDERS_PROJECTION_NAME_INDEX));
                            values.put(Bookmarks.DATE_CREATED,
                                       c.getInt(BOOKMARK_FOLDERS_PROJECTION_DATE_INDEX));
                            values.put(Bookmarks.POSITION, position);
                            values.put(Bookmarks.IS_FOLDER, true);
                            values.put(Bookmarks.PARENT,
                                       c.getInt(BOOKMARK_FOLDERS_PROJECTION_PARENT_ID_INDEX) + 1);
                            db.insertOrThrow(TABLE_BOOKMARKS, Bookmarks.DIRTY, values);
                        }
                        c.close();
                    }
                    /// @}
                    String table = BrowserProvider.
                                       TABLE_NAMES[BrowserProvider.URI_MATCH_BOOKMARKS];
                    // Import bookmarks
                    c = oldDb.query(table,
                            new String[] {
                            ...
                            Browser.BookmarkColumns.TOUCH_ICON, // 3
                    ...
                    if (c != null) {
                        while (c.moveToNext()) {
                            ...
                            /// M: add for feature bookmark folders
                            values.put(Bookmarks.POSITION, position);
                            //when import folder, folder id has increase 1,
                            //do it here too for matching them .
        ...
        private void createDefaultBookmarks(SQLiteDatabase db) throws Exception {
            ContentValues values = new ContentValues();
            ...
            if ((position = Extensions.getBookmarkPlugin(getContext())
                    .addDefaultBookmarksForCustomer(db)) == 0) {
    ...
    /// M: modification for site navigation
    //Bring it out from above to make it accessible to other class and packages
    public static CharSequence replaceSystemPropertyInString(Context context,
                                                             CharSequence srcString) {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case HOMEPAGE:
                ...
                try {
                    homepage = getHomePage(getContext());
                } catch (Exception e) {
                    e.printStackTrace();
                }
                if (null == homepage) {
    ...
    private Cursor doSuggestQuery(String selection, String[] selectionArgs, String limit) {
        ...
        if (TextUtils.isEmpty(selectionArgs[0])) {
            ...
            if (selectionArgs[0].startsWith("http")
                ...
                selection = "url LIKE ? OR url LIKE ? OR url LIKE ? OR url LIKE ?"
                    + " OR title LIKE ?";
                Log.i(XLOGTAG, "doSuggestQuery, selectionArgs: " + selectionArgs);
                Cursor mCursor = mOpenHelper.getReadableDatabase().query(
                        VIEW_OMNIBOX_SUGGESTIONS, projection,
                                selection, selectionArgs, null, null, null, null);
                SuggestionsCursor sc = new SuggestionsCursor(mCursor);
                Log.i(XLOGTAG, "doSuggestQuery, getCount: " + sc.getCount());
                return sc;
            }
        }
    ...
    public int deleteInTransaction(Uri uri, String selection, String[] selectionArgs,
            boolean callerIsSyncAdapter) throws Exception {
        final int match = URI_MATCHER.match(uri);
        final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        int deleted = 0;
        switch (match) {
    ...
    @Override
    public Uri insertInTransaction(Uri uri, ContentValues values, boolean callerIsSyncAdapter) throws Exception {
        int match = URI_MATCHER.match(uri);
        final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        long id = -1;
        if (match == LEGACY) {
            // Intercept and route to the correct table
            Integer bookmark = values.getAsInteger(Browser.BookmarkColumns.BOOKMARK);
            values.remove(Browser.BookmarkColumns.BOOKMARK);
            if (bookmark == null || bookmark == 0) {
                match = HISTORY;
            } else {
                match = BOOKMARKS;
                values.remove(Browser.BookmarkColumns.DATE);
                values.remove(Browser.BookmarkColumns.VISITS);
                values.remove(Browser.BookmarkColumns.USER_ENTERED);
        ...
        switch (match) {
            case BOOKMARKS: {
                if (values.containsKey(Bookmarks.URL)) {
                    String originalUrl = values.getAsString(Bookmarks.URL);
                    if (originalUrl != null) {
                        values.put(Bookmarks.URL, originalUrl.trim());
                    }
                }
                ...
                /// M: If add a folder to bookmark,
                /// make sure parent folder does not contains a same folder. @{
                if (values.containsKey(Bookmarks.IS_FOLDER)) {
                    if (values.getAsBoolean(Bookmarks.IS_FOLDER)
                            && values.containsKey(Bookmarks.PARENT)
    ...
    public int updateInTransaction(Uri uri, ContentValues values, String selection,
            String[] selectionArgs, boolean callerIsSyncAdapter) throws Exception {
        int match = URI_MATCHER.match(uri);
        final SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        if (match == LEGACY || match == LEGACY_ID) {
            // Intercept and route to the correct table
    ...
    private String getHomePage(Context context) throws Exception {
