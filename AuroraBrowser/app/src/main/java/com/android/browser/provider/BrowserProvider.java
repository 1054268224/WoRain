...
public class BrowserProvider extends ContentProvider {
    ...
    static {
        ...
        URI_MATCHER.addURI("MtkBrowserProvider", TABLE_NAMES[URI_MATCH_BOOKMARKS],
                URI_MATCH_BOOKMARKS);
        URI_MATCHER.addURI("MtkBrowserProvider", TABLE_NAMES[URI_MATCH_BOOKMARKS] + "/#",
                URI_MATCH_BOOKMARKS_ID);
        URI_MATCHER.addURI("MtkBrowserProvider", TABLE_NAMES[URI_MATCH_SEARCHES],
                URI_MATCH_SEARCHES);
        URI_MATCHER.addURI("MtkBrowserProvider", TABLE_NAMES[URI_MATCH_SEARCHES] + "/#",
                URI_MATCH_SEARCHES_ID);
        /// M: add for feature bookmark folders @{
        URI_MATCHER.addURI("MtkBrowserProvider", TABLE_NAMES[URI_MATCH_BOOKMARK_FOLDERS],
                URI_MATCH_BOOKMARK_FOLDERS);
        URI_MATCHER.addURI("MtkBrowserProvider", TABLE_NAMES[URI_MATCH_BOOKMARK_FOLDERS] + "/#",
                URI_MATCH_BOOKMARK_FOLDERS_ID);
        /// @}
        URI_MATCHER.addURI("MtkBrowserProvider", SearchManager.SUGGEST_URI_PATH_QUERY,
                URI_MATCH_SUGGEST);
        URI_MATCHER.addURI("MtkBrowserProvider",
    ...
    private static CharSequence replaceSystemPropertyInString(Context context,
    ...
    static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            try {
                for (int i = 0; i < size; i = i + 2) {
                    CharSequence bookmarkDestination
                                 = replaceSystemPropertyInString(mContext, bookmarks[i + 1]);
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion < 22) {
                db.execSQL("DELETE FROM bookmarks WHERE" +
                           "(bookmark = 0 AND url LIKE \"%.google.%client=ms-%\")");
                removeGears();
            }
            if (oldVersion < 23) {
    ...
    private Cursor doSuggestQuery(String selection, String[] selectionArgs, boolean bookmarksOnly) throws Exception {
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        if (isBookmarkTable
                && initialValues.containsKey(Browser.BookmarkColumns.BOOKMARK)
                && initialValues.getAsInteger(Browser.BookmarkColumns.BOOKMARK) != 0) {
    ...
    public int update(Uri url, ContentValues values, String where,
        ...
        if (match == URI_MATCH_BOOKMARKS_ID || match == URI_MATCH_BOOKMARKS) {
            ...
            if (values.containsKey(Browser.BookmarkColumns.BOOKMARK)) {
                changingBookmarks = true;
            } else if ((values.containsKey(Browser.BookmarkColumns.TITLE)
                     || values.containsKey(Browser.BookmarkColumns.URL))
                     && values.containsKey(Browser.BookmarkColumns._ID)) {
                ...
                Cursor cursor = cr.query(Browser.BOOKMARKS_URI,
                        ...
                        + values.getAsString(Browser.BookmarkColumns._ID), null, null);
                if (cursor.moveToNext()) {
    ...
    public static Cursor getBookmarksSuggestions(ContentResolver cr, String constraint) {
        Uri uri = Uri.parse("content://MtkBrowserProvider/" + SearchManager.SUGGEST_URI_PATH_QUERY);
