...
public class WebViewTimersControl {
    ...
    private Controller mRequestController;
    ...
    public void onBrowserActivityResume(WebView wv, Controller requestor) {
        if (LOGD_ENABLED) Log.d(LOGTAG, "onBrowserActivityResume");
        mBrowserActive = true;
        mRequestController = requestor;
        resumeTimers(wv);
    }

    public void onBrowserActivityPause(WebView wv, Controller requestor) {
        if (LOGD_ENABLED) Log.d(LOGTAG, "onBrowserActivityPause");
        if (requestor != null && !requestor.equals(mRequestController)){
            if (LOGD_ENABLED) Log.d(LOGTAG, "onBrowserActivityPause, controller =" + requestor
                    + " is not request resume timer or request pasue again.");
            return;
        }
