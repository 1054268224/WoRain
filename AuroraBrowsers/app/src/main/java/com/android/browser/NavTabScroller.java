...
public class NavTabScroller extends ScrollerView {
    ...
    interface OnRemoveListener {
        public void onRemovePosition(int position) throws Exception;
    }
    ...
    private void animateOut(final View v, float velocity, float start) {
        ...
        if (scroll != 0) {
            if (mHorizontal) {
                scroll1 = ObjectAnimator.ofInt(this, "scrollX", getScrollX(),
                                               getScrollX() + scroll);
            } else {
                scroll1 = ObjectAnimator.ofInt(this, "scrollY", getScrollY(),
                                               getScrollY() + scroll);
            }
        }
        if (translate != 0) {
