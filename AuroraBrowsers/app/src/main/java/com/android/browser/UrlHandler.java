...
public class UrlHandler {
    ...
    boolean startActivityForUrl(Tab tab, String url) {
        ...
        if (r == null) {
            ...
            if (packagename != null) {
                ...
                if (DEBUG) {
                    Log.d(TAG, "UrlHandler deeplink: " + url);
                }
                if (urlHasAcceptableScheme(url)) {
                    return false;
                }
                String keyWord = "S.browser_fallback_url=";
                int start = url.indexOf(keyWord);
                int end = -1;
                if (start > -1) {
                    end = url.indexOf(";", start);
                    if (end > -1) {
                        String downloadUrl = url.substring(start + keyWord.length(), end);
                        if (downloadUrl == null) {
                            downloadUrl = "https://m.baidu.com/";
                        }
                        if (downloadUrl.startsWith("%2F%2F")) {
                        ...
                        //tab.getWebView().loadUrl(downloadUrl);
                        ...
                        if (tab != null) {
                            if (tab.getAppId() == null) {
                                if (DEBUG) {
                                    Log.d(TAG, "UrlHandler.tabId = " + tab.getId());
                                }
                                tab.setAppId(mActivity.getPackageName() + "-" + tab.getId());
                            }
                            i.putExtra(com.android.browser.provider.Browser.EXTRA_APPLICATION_ID, tab.getAppId());
                        }
                        try {
                            mActivity.startActivity(i);
                        } catch (ActivityNotFoundException e) {
                            if (DEBUG) {
                                Log.w(TAG, "No activity found to handle " + downloadUrl);
                            }
                        }
                    }
                }
    ...
    boolean handleMenuClick(Tab tab, String url) throws Exception {
        if (DEBUG) {
