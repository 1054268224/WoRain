...
public class RequestHandler extends Thread {
    ...
    static {
        sUriMatcher.addURI(HomeProvider.AUTHORITY, null, INDEX);
    ...
    private static final String[] PROJECTION = new String[] {
        ...
        BrowserContract.History.THUMBNAIL
    };
    private static final String SELECTION = BrowserContract.History.URL
            + " NOT LIKE 'content:%' AND " + BrowserContract.History.THUMBNAIL + " IS NOT NULL";
    void writeTemplatedIndex() throws IOException {
        ...
        Cursor historyResults = mContext.getContentResolver().query(
                BrowserContract.History.CONTENT_URI, PROJECTION, SELECTION,
                null, BrowserContract.History.VISITS + " DESC LIMIT 12");
        Cursor cursor = historyResults;
        try {
            if (cursor.getCount() < 12) {
                Cursor bookmarkResults = mContext.getContentResolver().query(
                        BrowserContract.Bookmarks.CONTENT_URI, PROJECTION, SELECTION,
                        null, BrowserContract.Bookmarks.DATE_CREATED + " DESC LIMIT 12");
