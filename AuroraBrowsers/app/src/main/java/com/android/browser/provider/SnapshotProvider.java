...
public class SnapshotProvider extends ContentProvider {
    ...
    final static class SnapshotDatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            db.execSQL("CREATE TABLE " + TABLE_SNAPSHOTS + "(" +
                    ...
                    Snapshots.JOB_ID + " INTEGER," +
                    Snapshots.PROGRESS + " INTEGER," +
                    Snapshots.IS_DONE + " INTEGER" +
                    /// @ }
                    ");");
        }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        try {
            ...
            inserted = ContentUris.withAppendedId(uri, id);
    ...
    private void deleteSavePageDir(File file) {
        ...
        if (file.isDirectory()) {
            ...
            for (int i = 0; i < childFiles.length; i++) {
                deleteSavePageDir(childFiles[i]);
            }
            if (!file.delete()) {
                file.deleteOnExit();
            }
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        int count = 0;
        try {
            count = db.update(TABLE_SNAPSHOTS, values, selection, selectionArgs);
            getContext().getContentResolver().notifyChange(uri, null, false);
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
