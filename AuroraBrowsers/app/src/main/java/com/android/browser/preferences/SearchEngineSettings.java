...
import com.wheatek.reflect.SearchEngine;
...
public class SearchEngineSettings extends PreferenceFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (searchEngineName != null) {
            /*SearchEngineManager searchEngineManager = (SearchEngineManager) mActivity
                    .getSystemService(SearchEngineManager.SEARCH_ENGINE_SERVICE);*/
            SearchEngineManager searchEngineManager = new SearchEngineManager(getContext());
            List<SearchEngine> searchEngines = null;
            try {
                searchEngines = searchEngineManager.getAvailables();
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            for (int i = 0; i < len; i++) {
                ...
                try {
                    mEntries[i] = (String) searchEngines.get(i).getLabel();
                } catch (Exception e) {
                    e.printStackTrace();
                }
                try {
    ...
    public static List<SearchEngine> getSearchEngineInfos(Context context) throws Exception {
    ...
    public void onPause() {
        ...
        editor.putBoolean(PREF_SYNC_SEARCH_ENGINE, false);
        editor.commit();
        //if (mCheckBoxPref.isChecked()) {
        //    broadcastSearchEngineChangedExternal(mActivity);
        //}
     ...
     */
    private void broadcastSearchEngineChangedExternal(Context context) throws Exception {
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        editor.putString(BrowserSettings.PREF_SEARCH_ENGINE_FAVICON,
                         mEntryFavicon[preference.getOrder()]);
        editor.commit();
