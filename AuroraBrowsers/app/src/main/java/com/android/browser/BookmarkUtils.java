...
public class BookmarkUtils {
    ...
    static void createShortcutToHome(Context context, String url, String title,
            Bitmap touchIcon, Bitmap favicon) {
        ShortcutManager shortcutMgr = context.getSystemService(ShortcutManager.class);
        if (shortcutMgr.isRequestPinShortcutSupported()) {
            Bitmap icon = createIcon(context, touchIcon, favicon,
                                     BookmarkIconType.ICON_HOME_SHORTCUT);
            ShortcutInfo info = new ShortcutInfo.Builder(context, "bookmark"+url.hashCode())
                                        .setShortLabel(title)
                                        .setIcon(Icon.createWithBitmap(icon))
                                        .setIntent(createShortcutIntent(url)).build();
            boolean result = shortcutMgr.requestPinShortcut(info, null);
            Log.d("TestShortcut", "isRequestPinShortcutSupported true." + result);
        } else {
                        ...
                        Runnable runnable = new Runnable() {
                            ...
                            private void deleteFoldBookmarks(long id) {
                                ...
                                while (cursor.moveToNext()) {
                                    deleteFoldBookmarks(cursor
                                            .getInt(BookmarksLoader.COLUMN_INDEX_ID));
                                }
                                cursor.close();
                            }
