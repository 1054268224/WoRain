...
public class BrowserHistoryPage extends Fragment
    ...
    void checkIfEmpty() {
        if (mAdapter.mMostVisited != null && mAdapter.mHistoryCursor != null) {
            ...
            if (mAdapter.isEmpty()) {
                if (xlarge) {
                    mRoot.findViewById(R.id.tab_history).setVisibility(View.GONE);
                } else {
                ...
                if (xlarge) {
                    mRoot.findViewById(R.id.tab_history).setVisibility(View.VISIBLE);
                } else {
    ...
    private OnItemClickListener mGroupItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(
            ...
            //Refer to CR ALPS00247715, we don't want to show the title
            //(Tablet only, doesn't affect Phone's behavior).
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        /// M: Create the history page option menu. @{
        try {
            mBrowserHistoryExt = Extensions.getHistoryPlugin(getActivity());
        } catch (Exception e) {
            e.printStackTrace();
        }
        mBrowserHistoryExt.createHistoryPageOptionsMenu(menu, inflater);
        /// @}
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        /// M: Handle the history menu item selected. @{
        try {
            mBrowserHistoryExt = Extensions.getHistoryPlugin(getActivity());
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (mBrowserHistoryExt.historyPageOptionsMenuItemSelected(item, getActivity())) {
            return true;
        }
        /// @}
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        /// M: Prepare the history menu item. @{
        try {
            mBrowserHistoryExt = Extensions.getHistoryPlugin(getActivity());
        } catch (Exception e) {
            e.printStackTrace();
        }
        mBrowserHistoryExt.prepareHistoryPageOptionsMenuItem(
    ...
    static class ClearHistoryTask extends Thread {
        ...
        @Override
        public void run() {
            com.android.browser.provider.Browser.clearHistory(mResolver);
            com.android.browser.provider.Browser.clearSearches(mResolver);
        }
    }
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.share_link_context_menu_id:
                com.android.browser.provider.Browser.sendString(activity, url,
