...
class Tab implements PictureListener {
    ...
    private int mPageError = 0;

    /// M: The error code of opening saved page failed
    private static final int ERROR_OPEN_SAVED_PAGE_FAILED = -20000;
    ...
    private DialogInterface.OnDismissListener mDialogListener =
            new DialogInterface.OnDismissListener() {
                public void onDismiss(DialogInterface d) {
                    /// M: Clear the error dialog status
                    mIsErrorDialogShown = false;
                    processNextError();
                }
            };
    private LinkedList<ErrorDialog> mQueuedErrors;
    /// M: Used to record the error dialog status
    ...
    private void queueError(int err, String desc) {
        ...
        /// M: Set the error description for opening saved page failed case. @{
        if (err == ERROR_OPEN_SAVED_PAGE_FAILED && (desc == null || desc.isEmpty())) {
            desc = mContext.getString(R.string.open_saved_page_failed);
        }
        ErrorDialog errDialog = new ErrorDialog(
                (err == WebViewClient.ERROR_FILE_NOT_FOUND ||
                 err == ERROR_OPEN_SAVED_PAGE_FAILED) ?
    ...
    private void showError(ErrorDialog errDialog) {
        /// M: Check the current error dialog status before showing it
        if (mInForeground && !mIsErrorDialogShown) {
            ...
            d.show();
            /// M: Save the error dialog status
    ...
    private final WebViewClient mWebViewClient = new WebViewClient() {
        ...
        public boolean onRenderProcessGone(WebView view, RenderProcessGoneDetail detail) {
            if (!detail.didCrash()) {
                Log.d(TAG, "onRenderProcessGone: killed by system !");
                try {
                ...
                Log.d(TAG, "onRenderProcessGone: Crash !");
                return false;
            }
        }
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            ...
            if (tc != null) {
                if (DEBUG) {
                    Log.d(TAG, "Network_Issue [" + tc.getTabPosition(Tab.this) + "/"
                            + tc.getTabCount() + "] onPageStarted url=" + url
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            if (tc != null) {
                if (DEBUG) {
                    Log.d(TAG, "Network_Issue [" + tc.getTabPosition(Tab.this) + "/"
                            + tc.getTabCount() + "] onPageFinished url=" + url);
                }
            }
            /// @}

            mDisableOverrideUrlLoading = false;
            if (!isPrivateBrowsingEnabled() && DEBUG) {
                LogTag.logPageFinishedLoading(
                        url, SystemClock.uptimeMillis() - mLoadStartTime);
            }
            syncCurrentState(view, url);
            /// M: add for fix download page url @ {
            if (mCurrentState.mIsDownload) {
            ...
            /// M: ALPS02404678 add for fix url is about:blank,icon incorrect @ {
        ...
        public void onReceivedError(WebView view, int errorCode,
            ...
            if (DEBUG) {
                Log.d(LOGTAG, "Network_Issue error code: " + errorCode + " url: " + failingUrl);
            }
            mPageError = errorCode;
            mWebViewController.sendErrorCode(errorCode, failingUrl);
        ...
        /**
         * Print log for network issue.
         */
        @Override
        public void onReceivedHttpError(WebView view, WebResourceRequest request,
                WebResourceResponse response) {
            if (request.isForMainFrame() && DEBUG && response != null) {
                Log.d(LOGTAG, "Network_Issue http error code: " + response.getStatusCode()
                    + " url: " + request.getUrl());
            }
        }
        ...
        public void onReceivedSslError(final WebView view,
            ...
            if (DEBUG) {
                Log.d(LOGTAG, "Network_Issue onReceivedSslError: " + error.toString());
            }
            if (!mInForeground) {
                handler.cancel();
                try {
       ...
       /**
        ...
        public void onReceivedLoginRequest(WebView view, String realm,
            ...
            new DeviceAccountLogin(mWebViewController.getActivity(),
                     view, Tab.this, mWebViewController)
    ...
    private void syncCurrentState(WebView view, String url) {
        ...
        if (DEBUG) {
            Log.d(TAG, "Tab.syncCurrentState()()--->url = " + url
                    + ", webview = " + view);
        }
        ...
        if (mPageError != 0 && mCurrentState.mOriginalUrl != view.getOriginalUrl()) {
    ...
    DeviceAccountLogin getDeviceAccountLogin() {
        if (DEBUG) {
            Log.d(TAG, "Tab.getDeviceAccountLogin()--->");
        }
    ...
    // -------------------------------------------------------------------------
    private boolean mSubWindowShown = false;
    public void PopupWindowShown(boolean shown) {
        mSubWindowShown = shown;
    }

    private final WebChromeClient mWebChromeClient = new WebChromeClient() {
        // Helper method to create a new tab or sub window.
        private void createWindow(final boolean dialog, final Message msg) throws Exception {
        ...
        public void onReceivedTouchIconUrl(WebView view, String url,
            ...
            /// M: Modify for multi-thread, DownloadTouchIcon may set mTouchIconLoader to null.
        ...
        public boolean onConsoleMessage(ConsoleMessage consoleMessage) {
            ...
            //Don't log console messages if debug is disabled
    ...
    // Construct a new tab
    Tab(WebViewController wvcontroller, WebView w) throws Exception {
        this(wvcontroller, w, null);
    }

    Tab(WebViewController wvcontroller, Bundle state) throws Exception {
        this(wvcontroller, null, state);
    }

    Tab(WebViewController wvcontroller, WebView w, Bundle state) throws Exception {
        if (DEBUG) {
            Log.d(TAG, "Tab()--->Constructor()--->WebView is "
                + (w == null ? "null" : "not null")
                + ", Bundle is " + (state == null ? "null" : "not null"));
        }
        /// M: add for save page
        ...
        mDownloadListener = new BrowserDownloadListener() {
            public void onDownloadStart(String url, String userAgent,
                ...
                /// M: workaround for v74 WebView issue.
                if (!url.equalsIgnoreCase(mDownloadUrl)) {
                    if (DEBUG) {
                        Log.d(TAG, "no need download: " + mDownloadUrl);
                    }
                    return;
                }
                mDownloadUrl = null;
                /// M: Fix alps02340236, guess file's mime type @{
                mimetype = MimeTypeMap.getSingleton().remapGenericMimeTypePublic(mimetype, url,
                        contentDisposition);
                // @}
    ...
    void setWebView(WebView w) throws Exception {
        setWebView(w, true);
    }
     ...
     * the container view.
     */
    void setWebView(WebView w, boolean restore) throws Exception {
        if (DEBUG) {
            Log.d(TAG, "Tab.setWebView()--->webview = " + w + ", restore = " +  restore);
        }
        if (mMainView == w) {
            return;
        }
    ...
    void destroy() throws Exception {
        if (DEBUG) {
            Log.d(TAG, "Tab.destroy--->" + mMainView);
        }
        if (mMainView != null) {
            dismissSubWindow();
        ...
        if (mSavePageJob.size() != 0) {
            Toast.makeText(mContext, R.string.saved_page_failed, Toast.LENGTH_LONG).show();
    ...
    private class CancelSavePageTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            if (DEBUG) {
                Log.d(TAG, "Tab()--->CancelSavePageTask()--->doInBackground()");
            }
            Notification.Builder builder = new Notification.Builder(mContext);
            NotificationManager nm = (NotificationManager) mContext.getSystemService(
                                                        Context.NOTIFICATION_SERVICE);

            Set<Map.Entry<Integer, Long>> items = mSavePageJob.entrySet();
            Iterator<Map.Entry<Integer, Long>> iter = items.iterator();
            ...
            while (iter.hasNext()) {
                ...
                builder.setContentText(mContext.getText(R.string.saved_page_failed));
                builder.setOngoing(false);
                builder.setContentIntent(null);
                builder.setTicker(mContext.getText(R.string.saved_page_failed));

                nm.notify(job, builder.build());
                opList.add(ContentProviderOperation.newDelete(
                          ContentUris.withAppendedId(Snapshots.CONTENT_URI, id)).build());
            }
            try {
                mContext.getContentResolver().applyBatch(SnapshotProvider.AUTHORITY, opList);
            } catch (OperationApplicationException ope) {
                Log.e(LOGTAG, "Failed to delete save page. OperationApplicationException: "
                      + ope.getMessage());
            } catch (RemoteException re) {
                Log.e(LOGTAG, "Failed to delete save page. RemoteException: " + re.getMessage());
            }
            return null;
        }
    ...
    void removeFromTree() {
        if (DEBUG) {
            Log.d(TAG, "Tab.removeFromTree()--->tab this = " + this);
        }
        // detach the children
        if (mChildren != null) {
            for(Tab t : mChildren) {
     ...
     */
    boolean createSubWindow() throws Exception {
        if (DEBUG) {
            Log.d(TAG, "Tab.createSubWindow()--->mSubView = " + mSubView);
        }
        if (mSubView == null) {
            ...
            mSubView.setDownloadListener(new BrowserDownloadListener() {
                public void onDownloadStart(String url, String userAgent,
                    ...
                    mDownloadUrl = null;
                    /// M: Fix alps02340236, guess file's mime type @{
                    mimetype = MimeTypeMap.getSingleton().remapGenericMimeTypePublic(mimetype,
    ...
    void dismissSubWindow() {
        if (DEBUG) {
            Log.d(TAG, "Tab.dismissSubWindow()--->mSubView = " + mSubView);
        }
        if (mSubView != null) {
    ...
    void addChildTab(Tab child) {
        if (DEBUG) {
            Log.d(TAG, "Tab.addChildTab()--->Tab child = " + child);
        }
        if (mChildren == null) {
     ...
     * the controller.
     */
    private void setSecurityState(SecurityState securityState) throws Exception {
    ...
    public Bundle saveState() {
        ...
        if ((savedList == null || savedList.getSize() == 0) && DEBUG) {
    ...
    private void restoreState(Bundle b) {
        if (DEBUG) {
            Log.d(TAG, "Tab.restoreState()()---> bundle is " + b);
        }
        mSavedState = b;
        if (mSavedState == null) {
            return;
    ...
    public void updateBookmarkedStatus() {
        ...
        if (mCurrentState.mUrl != null &&
            mCurrentState.mUrl.equals(SiteNavigation.SITE_NAVIGATION_URI.toString())) {
            mDataController.queryBookmarkStatus(SiteNavigation.SITE_NAVIGATION_URI.toString(),
                                                mIsBookmarkCallback);
        } else {
    ...
    public ContentValues createSavePageContentValues(int id, String file) {
        if (DEBUG) {
            Log.d(TAG, "Tab.createSavePageContentValues()()--->"
                + "id = " + id + ", file = " + file);
        }
        if (mMainView == null)
            return null;
        ...
        values.put(Snapshots.PROGRESS, 0);
        values.put(Snapshots.IS_DONE, 0);
        values.put(Snapshots.JOB_ID, id);
        values.put(Snapshots.VIEWSTATE_PATH, file);
        return values;
    }
    /// @}
    ...
    public ContentValues createSnapshotValues() {
        ...
        String base_path = mContext.getFilesDir() + "/";
        if (DEBUG) {
            Log.d(TAG, "Tab.createSnapshotValues()--->base_path = " + base_path);
        }
        mMainView.saveWebArchive(base_path, true, callback);
        return values;
    }
    ...
    public boolean saveViewState(ContentValues values) {
        ...
        try {
            ...
            // TODO: Check the save fail handle
            return false;
        }
        if (callback.mResult == null) {
            return false;
        }
        File f = new File(callback.mResult);
        values.put(Snapshots.VIEWSTATE_PATH, callback.mResult);
        values.put(Snapshots.VIEWSTATE_SIZE, f.length());
        return true;
    }
    ...
    public void loadUrl(String url, Map<String, String> headers) throws Exception {
        if (DEBUG) {
            Log.d(TAG, "Tab.loadUrl()()--->url = " + url
                    + ", headers = " + headers + ", mDownloadUrl = " + mDownloadUrl);
        }
        /// M: workaround for v74 WebView issue.
        mDownloadUrl = url;
        if (mMainView != null) {
            /// M: Check whether the url is too long, if yes, trim the url. @{
            mBrowserUrlExt = Extensions.getUrlPlugin(mContext);
            url = mBrowserUrlExt.checkAndTrimUrl(url);
            /// @}
            mPageLoadProgress = INITIAL_PROGRESS;
            mInPageLoad = true;
            ...
            headers.put(Browser.HEADER, Browser.UAPROF);
    ...
    protected void capture() {
        ...
        if (DEBUG) {
            Log.d(TAG, "Tab.capture()--->left = "
                + left + ", top = " + top + ", scale = " + scale);
        }
    ...
    public void onNewPicture(WebView view, Picture picture) {
        // / M: alps03022427 Avoid UI refreshing in tab navigation screen. @{
        if (mWebViewController instanceof Controller) {
            Controller c = (Controller) mWebViewController;
            if (c.getUi().isWebShowing()) {
                postCapture();
            }
        }
        // /@}
    ...
    void updateCaptureFromBlob(byte[] blob) {
        synchronized (Tab.this) {
            ...
            Bitmap temp = BitmapFactory.decodeByteArray(buffer.array(),
                    buffer.arrayOffset(), buffer.capacity(), opt);
            if (temp == null) {
                return;
            }
            try {
                mCapture = Bitmap.createScaledBitmap(temp, mCapture.getWidth(),
                        mCapture.getHeight(), true);
    ...
    private void handleProceededAfterSslError(SslError error) throws Exception {
    ...
    void addDatabaseItemId(int job, long id) {
        ...
        mSavePageJob.put(job, id);
    }

    Long getDatabaseItemId(int job) {
        if (mSavePageJob != null) {
            return mSavePageJob.get(job);
        }
        return null;
    }
    ...
    void removeDatabaseItemId(int job) {
        if (mSavePageJob != null) {
            mSavePageJob.remove(job);
        }
    }

    void clearDatabaseItemId() {
        if (mSavePageJob != null) {
            mSavePageJob.clear();
        }
    }
    ...
    boolean containsDatabaseItemId(int job) {
        if (mSavePageJob != null) {
            return mSavePageJob.containsKey(job);
        }
        return false;
    }
    /// @ }
