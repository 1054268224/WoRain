...
public class BookmarkThumbnailWidgetService extends RemoteViewsService {
    ...
    static void changeFolder(Context context, Intent intent) {
        ...
        long fid = intent.getLongExtra(BrowserContract.Bookmarks._ID, -1);
        if (wid >= 0 && fid >= 0) {
    ...
    static class BookmarkFactory implements RemoteViewsService.RemoteViewsFactory {
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (isFolder) {
                fillin = new Intent(ACTION_CHANGE_FOLDER)
                        ...
                        .putExtra(BrowserContract.Bookmarks._ID, id);
            } else {
        ...
        void loadBookmarks() {
            ...
            if (mCurrentFolder != mRootFolder) {
                ...
                if (mBookmarks.getCount() == 0) {
                    String select = BrowserContract.Bookmarks.PARENT + "=?";
                    String[] args = new String[]{ String.valueOf(mRootFolder) };
                    Cursor cursor = mContext.getContentResolver()
                                    .query(BrowserContract.Bookmarks.CONTENT_URI, PROJECTION,
                                           select, args, null);
                    mBookmarks = new MergeCursor(new Cursor[] { cursor, mBookmarks });
                }
            }
        }
    }
