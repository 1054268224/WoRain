...
public class BrowserBookmarksPage extends Fragment implements View.OnCreateContextMenuListener,
    ...
    View mEmptyView;
    View mHeader;
    HashMap<Integer, BrowserBookmarksAdapter> mBookmarkAdapters
            = new HashMap<Integer, BrowserBookmarksAdapter>();
    JSONObject mState;
    long mCurrentFolderId = BrowserProvider2.FIXED_ID_ROOT;

    /// M: Browser bookmark plugin
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        /// M: Create bookmark option menu. @{
        try {
            mBrowserBookmarkExt = Extensions.getBookmarkPlugin(getActivity());
        } catch (Exception e) {
            e.printStackTrace();
        }
        mBrowserBookmarkExt.createBookmarksPageOptionsMenu(menu, inflater);
        /// @}
    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        /// M: Handle the bookmark menu item selected. @{
        try {
            mBrowserBookmarkExt = Extensions.getBookmarkPlugin(getActivity());
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (mBrowserBookmarkExt.bookmarksPageOptionsMenuItemSelected(
                item, getActivity(), mCurrentFolderId)) {
            return true;
        }
        ///@}
    ...
    public boolean handleContextItem(int itemId, int groupPosition,
        ...
        String bookmarkUrl = BrowserBookmarksPage.getUrl(adapter.getItem(childPosition));
        if (bookmarkUrl == null && (itemId == R.id.open_context_menu_id
                || itemId == R.id.copy_url_context_menu_id
                || itemId == R.id.share_link_context_menu_id
                || itemId == R.id.shortcut_context_menu_id)) {
            Toast.makeText(getActivity(), R.string.bookmark_url_not_valid,
    ...
    public void onDestroyView() {
        ...
        for (int id : mBookmarkAdapters.keySet()) {
            /// M: [ALPS02706992] fix cursor not synchronized issue @{
    ...
    /* package */ static void createShortcut(Context context, Cursor cursor) {
        String url = cursor.getString(BookmarksLoader.COLUMN_INDEX_URL);
        String title = cursor.getString(BookmarksLoader.COLUMN_INDEX_TITLE);
        ...
        BookmarkUtils.createShortcutToHome(context, url, title, touchIcon, favicon);
    }
    ...
    private void openInNewWindow(BrowserBookmarksAdapter adapter, int position) {
        if (mCallbacks != null) {
            ...
            if (isFolder) {
                ...
                new OpenAllInTabsTask(id).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            } else {
    ...
    /// M: modify for open bookmarks in sub folders @ {
    /** open bookmarks in sub folders. */
    class OpenAllInTabsTask extends AsyncTask<Void, Void, ArrayList<String>> {
        ...
        private void getChildrenUrls(Context c, long id) {
            Cursor cursor = c.getContentResolver().query(BookmarkUtils.getBookmarksUri(c),
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    if (cursor.getInt(BookmarksLoader.COLUMN_INDEX_IS_FOLDER) == 0) {
                        mUrls.add(cursor.getString(BookmarksLoader.COLUMN_INDEX_URL));
                    } else {
                        getChildrenUrls(c, cursor.getLong(BookmarksLoader.COLUMN_INDEX_ID));
                    }
                }
                cursor.close();
            }
        ...
        protected void onPostExecute(ArrayList<String> result) {
            if (result != null && result.size() == 0) {
                ...
                Toast.makeText(ctx, ctx.getString(R.string.contextheader_folder_empty),
                        Toast.LENGTH_LONG).show();
            } else if (mCallbacks != null && (result != null && result.size() > 0)) {
                mCallbacks.onOpenInNewWindow(mUrls.toArray(new String[0]));
            }
        }
    }
    /// @ }
    ...
    private void editBookmark(BrowserBookmarksAdapter adapter, int position) {
        ...
        if (data != null) {
            ...
            if (icon != null && icon.getWidth() > Controller.LIMIT_FAVICON_HIGHT_WIDTH) {
                icon = Bitmap.createScaledBitmap(icon,
                                                 Controller.LIMIT_FAVICON_HIGHT_WIDTH,
                                                 Controller.LIMIT_FAVICON_HIGHT_WIDTH,
                                                 true);
            }
            item.putParcelable(BrowserContract.Bookmarks.FAVICON, icon);
        }
    ...
    private void copy(CharSequence text) {
        ...
        if (url.startsWith("content:")) {
            ...
            cm.setPrimaryClip(ClipData.newRawUri(null, Uri.parse(url)));
        }
    }
        ...
        protected Integer doInBackground(Long... params) {
            ...
            try {
                c = mContext.getContentResolver().query(uri,
                        null, BrowserContract.Bookmarks.PARENT + "=? AND "
    ...
    static class AccountsLoader extends CursorLoader {
        ...
        public AccountsLoader(Context context) {
            super(context, Accounts.CONTENT_URI
                    .buildUpon()
                    .appendQueryParameter(BrowserProvider2.PARAM_ALLOW_EMPTY_ACCOUNTS, "true")
                    .build(),
