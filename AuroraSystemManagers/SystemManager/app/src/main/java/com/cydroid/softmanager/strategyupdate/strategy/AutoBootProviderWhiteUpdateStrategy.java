 ...
 */
public class AutoBootProviderWhiteUpdateStrategy implements IStrategy {
    private static final String TAG = AutoBootProviderWhiteUpdateStrategy.class.getSimpleName();
    public static final String TYPE = "autobootprovider";
    private final static String AUTOBOOT_PROVIDER_VERSION_TAG = "autoboot_provider_version";
    ...
    public void parseJson(Context context, String json, ParseStrategyCallBack callBack) {
        ...
        try {
            ...
            if (dataArray == null || !isUpdateAutoBootProviderWhiteList()) {
                return;
            }
            context.getContentResolver().delete(Consts.ROSTER_CONTENT_URI, "usertype='" + TYPE + "'", null);
            updateAutoBootProviderWhiteList(context, dataArray, callBack);
            mNeedNotify = true;
            if (callBack != null) {
    ...
    private void updateVersion(Context context, JSONObject jsonObj) throws JSONException {
        ...
        Settings.System.putInt(context.getContentResolver(), AUTOBOOT_PROVIDER_VERSION_TAG, version);
    }

    private void updateAutoBootProviderWhiteList(Context context, JSONArray dataArray,
    ...
    private boolean isUpdateAutoBootProviderWhiteList() {
        File file = new File(Environment.getExternalStorageDirectory(),
                "testAutoBootProviderWhiteList9876543210");
        if (file.exists()) {
            Log.d(TAG, "isUpdateAutoBootProviderWhiteList testAutoBootProviderWhiteList9876543210 exists");
            return false;
        }
        return true;
    }
    ...
    public int getVersion(Context context) {
        final int VERSION = Settings.System.getInt(context.getContentResolver(),
                AUTOBOOT_PROVIDER_VERSION_TAG, 0);
        return VERSION;
    }
