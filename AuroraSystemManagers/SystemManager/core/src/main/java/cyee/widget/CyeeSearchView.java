 ...
 */
public class CyeeSearchView extends LinearLayout implements CollapsibleActionView {
...
//    private View mVoiceButton;
    ...
    private final Drawable mQueryTextViewBackground;
    ...
    private final Runnable mShowImeRunnable = new Runnable() {
        public void run() {
            ...
            if (imm != null) {
                // Gionee <lihq> <2013-8-29> modify for CR00871765 begin
                imm.showSoftInputUnchecked(0, null);
//                imm.showSoftInput(CyeeSearchView.this, 0, null);
//                imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
//                imm.showSoftInput(mQueryTextView, InputMethodManager.SHOW_FORCED);
                // Gionee <lihq> <2013-8-29> modify for CR00871765 end
            }
        }
    };
    ...
    private final Runnable mReleaseCursorRunnable = new Runnable() {
        public void run() {
            if (mSuggestionsAdapter != null && mSuggestionsAdapter instanceof CyeeSuggestionsAdapter) {
                /// M: When release cursor, just close it.
                ((CyeeSuggestionsAdapter) mSuggestionsAdapter).close();
            }
        }
    };
    ...
    public CyeeSearchView(Context context) {
        this(context, null);
    }

    public CyeeSearchView(Context context, AttributeSet attrs) {
        super(context, attrs);

        // gionee maxw add begin
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CyeeSearchView, R.attr.cyeeSearchViewStyle, 0);
        mTextColor = typedArray.getColorStateList(R.styleable.CyeeSearchView_cyeeTextColor);
        mHintColor = typedArray.getColorStateList(R.styleable.CyeeSearchView_cyeeHintColor);
        mQueryTextViewBackground = typedArray.getDrawable(R.styleable.CyeeSearchView_cyeeBackground);
        mVoiceIcon = typedArray.getDrawable(R.styleable.CyeeSearchView_cyeeVoiceIcon);
        isDarkTheme = typedArray.getBoolean(R.styleable.CyeeSearchView_cyeeDarkTheme, false);
        mMarginEnd = typedArray.getDimension(R.styleable.CyeeSearchView_cyeeMarginEnd, getResources().getDimension(com.cyee.internal.R.dimen.cyee_searchview_edittext_margin_end));
        mQRCodeIcon = typedArray.getDrawable(R.styleable.CyeeSearchView_cyeeQRCodeIcon);
        typedArray.recycle();
        // gionee maxw add end
        ...
        // inflater.inflate(com.cyee.internal.R.layout(this,"search_view, this, true"));
        inflater.inflate(com.cyee.internal.R.layout.cyee_search_view, this, true);		
        // Gionee zhangxx 2012-11-05 add for CR00715173 begin

        mSearchButton = findViewById(com.cyee.internal.R.id.cyee_search_button);
        mQueryTextView = (SearchAutoComplete) findViewById(com.cyee.internal.R.id.cyee_search_src_text);
        mQueryTextView.setSearchView(this);

        mSearchEditFrame = findViewById(com.cyee.internal.R.id.cyee_search_edit_frame);
        mSearchPlate = findViewById(com.cyee.internal.R.id.cyee_search_plate);
//        mSubmitArea = findViewById(R.id.cyee_submit_area);
//        mSubmitButton = findViewById(R.id.cyee_search_go_btn);
        mCloseButton = (ImageView) findViewById(com.cyee.internal.R.id.cyee_search_close_btn);

        setDefaultTheme(isDarkTheme);
        //        mVoiceButton = findViewById(R.id.cyee_search_voice_btn);
        ...
        mQueryTextView.setOnFocusChangeListener(new OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (mOnQueryTextFocusChangeListener != null) {
                    mOnQueryTextFocusChangeListener.onFocusChange(CyeeSearchView.this, hasFocus);
                }
            }
        });

        // Gionee <gaoj> <2013-9-19> add for CR00893366 begin
        mQueryTextView.setDropDownWidth(ViewGroup.LayoutParams.WRAP_CONTENT);
        // Gionee <gaoj> <2013-9-19> add for CR00893366 end
        
        TypedArray a = context.obtainStyledAttributes(attrs, android.R.styleable.SearchView, 0, 0);
//        setIconifiedByDefault(a.getBoolean(android.R.styleable.SearchView_iconifiedByDefault, true));
        ...
        mIsGnWidget3Style = isGioneeViewStyle() && isGioneeWidget3Support();
        if (mIsGnWidget3Style) {
            initSearchView();
            // Gionee <lihq> <2013-8-29> modify for CR00873484 begin
            // Gionee <fenglp> <2013-08-01> add for CR00812456 begin
            //startBeginAnimation();
            // Gionee <fenglp> <2013-08-01> add for CR00812456 end
            // Gionee <lihq> <2013-8-29> modify for CR00873484 end        
        }
    ...
    private void changeColors() {
        if(ChameleonColorManager.getInstance().getCyeeThemeType(mContext) == ChameleonColorManager.CyeeThemeType.GLOBAL_THEME) {
            mVoiceIcon = getResources().getDrawable(com.cyee.internal.R.drawable.cyee_global_theme_search_voice_icon);
            Drawable closeDrawable = getResources().getDrawable(com.cyee.internal.R.drawable.cyee_global_theme_search_close_icon_selector);
            mCloseButton.setImageDrawable(closeDrawable);
            if(mGnSearchBgLayout != null) {
                Drawable bgDrawable = getResources().getDrawable(com.cyee.internal.R.drawable.cyee_global_theme_input_circle_background);
                mGnSearchBgLayout.setBackground(bgDrawable);
            }
        } else if (ChameleonColorManager.isNeedChangeColor(mContext)) {
            mCloseButton.getDrawable().setColorFilter(
                    ChameleonColorManager.getContentColorPrimaryOnAppbar_T1(),
                    android.graphics.PorterDuff.Mode.SRC_IN);

            changeSelectableItemBackground(mCloseButton);
            if (mGnSearchBgLayout != null) {
                // Gionee <weidong> <2017-07-21> add for 166855 begin
                GradientDrawable gDrawable = new GradientDrawable();
                gDrawable.setColor(ChameleonColorManager.getEditTextBackgroudColor_B3());
                gDrawable.setCornerRadius(getResources().getDimension(
                        com.cyee.internal.R.dimen.cyee_searchview_edittext_bg_corner));
                mGnSearchBgLayout.setBackground(gDrawable);
                // Gionee <weidong> <2017-07-21> add for 166855 end
            }
        } else {
            if(mGnSearchBgLayout != null) {
                mGnSearchBgLayout.setBackground(mQueryTextViewBackground);
            }
        }
        if (mGnSearchBgLayout != null) {
            ViewGroup.MarginLayoutParams marginParam = (ViewGroup.MarginLayoutParams) mGnSearchBgLayout.getLayoutParams();
            marginParam.setMarginEnd((int)mMarginEnd);
        }
    }
     ...
     * Set Default Theme
     * 
     *  @param isDarkTheme is default theme is dark theme
     */
    public void setDefaultTheme(boolean isDarkTheme) {
        ...
        if (ChameleonColorManager.isNeedChangeColor(mContext)) {
            if(ChameleonColorManager.getThemeType() == 2) {
                mCloseButton.setImageResource(com.cyee.internal.R.drawable.cyee_ic_clear_dark);
            }
        } else {
            if(isDarkTheme) {
                mCloseButton.setImageResource(com.cyee.internal.R.drawable.cyee_ic_clear_dark);
    ...
    public void setFilters(InputFilter[] filters) {
        mQueryTextView.setFilters(filters);
    }
    ...
    public void setQueryRefinementEnabled(boolean enable) {
        ...
        if (mSuggestionsAdapter instanceof CyeeSuggestionsAdapter) {
            ((CyeeSuggestionsAdapter) mSuggestionsAdapter).setQueryRefinement(
                    enable ? CyeeSuggestionsAdapter.REFINE_ALL :CyeeSuggestionsAdapter.REFINE_BY_ENTRY);
        }
    }
    ...
    private int getPreferredWidth() {
        return getContext().getResources()
                .getDimensionPixelSize(com.cyee.internal.R.dimen.cyee_search_view_preferred_width);
    }
...
//        mSubmitArea.setVisibility(visibility);
    ...
    protected void onAttachedToWindow() {
        ...
        if (mSuggestionsAdapter != null && mSuggestionsAdapter instanceof CyeeSuggestionsAdapter) {
            ((CyeeSuggestionsAdapter) mSuggestionsAdapter).enable();
        }
        // Gionee <zhangxx><2013-05-09> add for CR00811583 begin
        startBeginAnimation();
        // Gionee <zhangxx><2013-05-09> add for CR00811583 end
    ...
    /**
     * Called by the CyeeSuggestionsAdapter
     * @hide
     */
    ...
    private static String getActionKeyMessage(Cursor c, SearchableInfo.ActionKeyInfo actionKey) {
        ...
        if (column != null) {
            result = CyeeSuggestionsAdapter.getColumnString(c, column);
        }
...
//        TypedValue outValue = new TypedValue();
//        
//        boolean rtn = getContext().getTheme().resolveAttribute(com.android.internal.R.attr.noSearchIconInSearchView,
//                outValue, true);
...
//        TypedValue outValue = new TypedValue();
//        
//        boolean rtn = getContext().getTheme().resolveAttribute(com.android.internal.R.attr.SearchViewLayout,
//                outValue, true);
//        return outValue.resourceId <= 0 ? R.layout.cyee_search_view : outValue.resourceId;
//    }    
...
//      ssb.setSpan(new ImageSpan(searchIcon), 1, 2, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    private void updateSearchAutoComplete() {
        ...
        if (mSearchable.getSuggestAuthority() != null) {
            mSuggestionsAdapter = new CyeeSuggestionsAdapter(getContext(),
                    this, mSearchable, mOutsideDrawablesCache);
            mQueryTextView.setAdapter(mSuggestionsAdapter);
            ((CyeeSuggestionsAdapter) mSuggestionsAdapter).setQueryRefinement(
                    mQueryRefinement ? CyeeSuggestionsAdapter.REFINE_ALL
                    : CyeeSuggestionsAdapter.REFINE_BY_ENTRY);
        }
    }
...
//    }
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(CyeeSearchView.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(CyeeSearchView.class.getName());
    }
    ...
    private void adjustDropDownSizeAndPosition() {
        if (mDropDownAnchor.getWidth() > 1) {
            ...
            int iconOffset = mIconifiedByDefault
                    ? res.getDimensionPixelSize(com.cyee.internal.R.dimen.cyee_dropdownitem_icon_width)
                    + res.getDimensionPixelSize(com.cyee.internal.R.dimen.cyee_dropdownitem_text_padding_left)
                    : 0;
    ...
    private final OnItemSelectedListener mOnItemSelectedListener = new OnItemSelectedListener() {
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            ...
            CyeeSearchView.this.onItemSelected(position);
        }
    ...
    private Intent createIntentFromSuggestion(Cursor c, int actionKey, String actionMsg) {
        try {
            ...
            String action = CyeeSuggestionsAdapter.getColumnString(c, SearchManager.SUGGEST_COLUMN_INTENT_ACTION);
            ...
            String data = CyeeSuggestionsAdapter.getColumnString(c, SearchManager.SUGGEST_COLUMN_INTENT_DATA);
            if (data == null) {
                data = mSearchable.getSuggestIntentData();
            }
            // then, if an ID was provided, append it.
            if (data != null) {
                String id = CyeeSuggestionsAdapter.getColumnString(c, SearchManager.SUGGEST_COLUMN_INTENT_DATA_ID);
                if (id != null) {
            ...
            String query = CyeeSuggestionsAdapter.getColumnString(c, SearchManager.SUGGEST_COLUMN_QUERY);
            String extraData = CyeeSuggestionsAdapter.getColumnString(c, SearchManager.SUGGEST_COLUMN_INTENT_EXTRA_DATA);
    ...
    private final TextWatcher mTextWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start,
            ...
            CyeeSearchView.this.onTextChanged(s);
        }
     ...
     * @hide
     */
    public static class SearchAutoComplete extends CyeeAutoCompleteTextView {

        private int mThreshold;
        private CyeeSearchView mSearchView;
...
//            mThreshold = getThreshold();
        ...
        void setSearchView(CyeeSearchView searchView) {
...
//                    }
    ...
    // Gionee <zhangxx><2013-05-09> add for CR00811583 begin
    private final int GN_SEARCHVIEW_ANIMTIME = 300;
    
    private final int VOICE_MODE = 1;
    private final int QRCODE_MODE = 2;
    private final int QRCODE_VOICE_MODE = 4;
    
    private boolean mIsGnWidget3Style = false;
    private boolean mIsSearchSubmitMode = false;
    ...
    // Gionee <lihq> <2014-4-22> add for CR00873172 begin
    private LinearLayout mSearchViewLayout;
    // Gionee <lihq> <2014-4-22> add for CR00873172 end
    private void initSearchView() {
        int count = 0;
        // Gionee <lihq> <2014-4-22> add modify CR00873172 begin
        mSearchViewLayout = (LinearLayout)findViewById(com.cyee.internal.R.id.cyee_search_bar);
        mGnSearchBgLayout = (LinearLayout)findViewById(com.cyee.internal.R.id.cyee_search_background);
        mSearchViewZoomAnim = AnimationUtils.loadAnimation(getContext(),com.cyee.internal.R.anim.cyee_searchview_zoom);
        //mSearchViewZoomAnim.setAnimationListener(mSearchAnimationListener);
        // Gionee <lihq> <2014-4-22> add modify CR00873172 end
        mSearchViewAnim = AnimationUtils.loadAnimation(getContext(), com.cyee.internal.R.anim.cyee_searchview_text);
        
        mGnSearchVoiceButton = (ImageView)findViewById(com.cyee.internal.R.id.cyee_search_voice_btn);
        mGnSearchGoButton = (ImageView)findViewById(com.cyee.internal.R.id.cyee_search_go_btn);
    
        mQRCodeButton = (ImageView) findViewById(com.cyee.internal.R.id.cyee_qr_code_btn);
        
        if (null != mQRCodeButton && null != mQRCodeIcon) {
            mQRCodeButton.setImageDrawable(mQRCodeIcon);
        }
        
        // gionee maxw add begin
        if(mGnSearchVoiceButton!=null && mVoiceIcon!=null) {
        	mGnSearchVoiceButton.setImageDrawable(mVoiceIcon);
        }
        // gionee maxw add end
    }
    ...
    public void setSubmitSearchMode(boolean isSubmitSearchMode, OnClickListener searchSubmitClickListener) {
        mIsSearchSubmitMode = isSubmitSearchMode;
        ...
        if (isSubmitSearchMode) {
        	// gionee maxw delete begin
            mGnSearchBgLayout.setBackgroundResource(com.cyee.internal.R.drawable.cyee_searchview_background_submit);
            // gionee maxw delete end
    ...
    public void setVoiceSearchMode(boolean isVoiceSearchMode, OnClickListener searchVoiceClickListener) {
        ...
        if (isVoiceSearchMode) {
            mSearchMode = VOICE_MODE;
        }
        mGnSearchVoiceButton.setVisibility(isVoiceSearchMode ? VISIBLE : GONE);
        mQRCodeButton.setVisibility(View.GONE);
        
        if (mSearchMode == VOICE_MODE) {
            mGnSearchVoiceButton.setOnClickListener(searchVoiceClickListener);
            if (ChameleonColorManager.isNeedChangeColor(mContext)) {
                mGnSearchVoiceButton.getDrawable().setColorFilter(
                        ChameleonColorManager.getContentColorPrimaryOnAppbar_T1(),
                        android.graphics.PorterDuff.Mode.SRC_IN);
                changeSelectableItemBackground(mGnSearchVoiceButton);
            }
        }
    }
    
    /**
     * set QRCode search mode
     * @param isQRCodeSearchMode true is QRCodemode; else is not
     * @param searchQRCodeClickListener
     */
    public void setQRCodeSearchMode(boolean isQRCodeSearchMode, OnClickListener searchQRCodeClickListener) {
        if (mQRCodeButton == null){
            return;
        }
        
        if (isQRCodeSearchMode) {
            mSearchMode = QRCODE_MODE;
        }
        
        mQRCodeButton.setVisibility(isQRCodeSearchMode ? VISIBLE : GONE);
        mGnSearchVoiceButton.setVisibility(View.GONE);
        
        if (mSearchMode == QRCODE_MODE) {
            mQRCodeButton.setOnClickListener(searchQRCodeClickListener);
            if (ChameleonColorManager.isNeedChangeColor(mContext)) {
                mQRCodeButton.getDrawable().setColorFilter(
                        ChameleonColorManager.getContentColorPrimaryOnAppbar_T1(),
                        android.graphics.PorterDuff.Mode.SRC_IN);
                changeSelectableItemBackground(mQRCodeButton);
            }
        }
    }
    
    /**
     * set searchmode as support qrcode and voicesearch
     * @param searchVoiceClickListener click voice search callback
     * @param qrCodeClickListener click qrcode callback
     */
    public void setQRAndVoiceSearchMode(OnClickListener searchVoiceClickListener, OnClickListener qrCodeClickListener) {
        if (mQRCodeButton == null) {
            setVoiceSearchMode(true, searchVoiceClickListener);
            return;
        }
        if (mGnSearchVoiceButton == null) {
            setQRCodeSearchMode(true, qrCodeClickListener);
            return;
        }

        mSearchMode = QRCODE_VOICE_MODE;
        mQRCodeButton.setVisibility(VISIBLE);
        mGnSearchVoiceButton.setVisibility(VISIBLE);

        mQRCodeButton.setOnClickListener(qrCodeClickListener);
        mGnSearchVoiceButton.setOnClickListener(searchVoiceClickListener);
        if (ChameleonColorManager.isNeedChangeColor(mContext)) {
            mQRCodeButton.getDrawable().setColorFilter(ChameleonColorManager.getContentColorPrimaryOnAppbar_T1(),
                    android.graphics.PorterDuff.Mode.SRC_IN);
            changeSelectableItemBackground(mQRCodeButton);

            mGnSearchVoiceButton.getDrawable().setColorFilter(ChameleonColorManager.getContentColorPrimaryOnAppbar_T1(),
                    android.graphics.PorterDuff.Mode.SRC_IN);
            changeSelectableItemBackground(mGnSearchVoiceButton);
        }
    }
    ...
    private void updateSearchVoiceButton(boolean isShow) {
        if (null != mGnSearchVoiceButton && mSearchMode == VOICE_MODE) {
            mGnSearchVoiceButton.setVisibility(isShow ? VISIBLE : GONE);
        } else if (null != mQRCodeButton && mSearchMode == QRCODE_MODE) {
            mQRCodeButton.setVisibility(isShow ? VISIBLE : GONE);
        } else if (mSearchMode == QRCODE_VOICE_MODE) {
            if (null != mGnSearchVoiceButton) {
                mGnSearchVoiceButton.setVisibility(isShow ? VISIBLE : GONE);
            }
    ...
    private void startBeginAnimation() {  
        // Gionee <lihq> <2014-4-22> modify for CR00873172 begin
        /*
        mSearchPlate.setVisibility(GONE);
        mGnSearchBgLayout.setVisibility(VISIBLE);
        if (mIsSearchSubmitMode && mGnSearchGoButton != null) {
            mGnSearchGoButton.setVisibility(INVISIBLE);
        }
        
        android.view.ViewGroup.LayoutParams lp = mGnSearchBgLayout.getLayoutParams();
        lp.width = android.view.ViewGroup.LayoutParams.WRAP_CONTENT;
        mGnSearchBgLayout.startAnimation(mSearchViewZoomAnim);
    	mSearchPlate.setVisibility(VISIBLE);
        mGnSearchBgLayout.setVisibility(VISIBLE);
        if (mIsSearchSubmitMode && mGnSearchGoButton != null) {
            mGnSearchGoButton.setVisibility(VISIBLE);
        }                
        mSearchPlate.startAnimation(mSearchViewAnim);  
        */
        mSearchViewLayout.startAnimation(mSearchViewZoomAnim);
        // Gionee <lihq> <2014-4-22> modify for CR00873172 end
    ...
    private class UnfoldAnimation extends Animation {
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            android.view.ViewGroup.LayoutParams lp = mGnSearchBgLayout.getLayoutParams();
            // Gionee <zhangxx> <2013-10-19> modify for CR00905403 begin
            if (interpolatedTime >= 1.0) {
            	lp.width = ViewGroup.LayoutParams.MATCH_PARENT;
            } else {
            // Gionee <zhangxx> <2013-10-19> modify for CR00905403 end
            	lp.width = (int)(mStartWidth + mDeltaWidth * interpolatedTime);
            // Gionee <zhangxx> <2013-10-19> modify for CR00905403 begin
            }
            // Gionee <zhangxx> <2013-10-19> modify for CR00905403 end
            mGnSearchBgLayout.setLayoutParams(lp);
        }
    ...
    private void setupSearchViewUnfoldAnim() {
        android.view.ViewGroup.MarginLayoutParams lp = (MarginLayoutParams)mSearchEditFrame.getLayoutParams();
        
        mSearchViewUnfoldAnim = new UnfoldAnimation(mGnSearchBgLayout.getWidth(), mSearchEditFrame.getWidth());
        mSearchViewUnfoldAnim.setAnimationListener(mSearchAnimationListener);
        mSearchViewUnfoldAnim.setInterpolator(getContext(), android.R.anim.decelerate_interpolator);
        mSearchViewUnfoldAnim.setDuration(GN_SEARCHVIEW_ANIMTIME);
    }
    
    private final AnimationListener mSearchAnimationListener = new AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation) {
            if (animation == mSearchViewZoomAnim) {
                mSearchPlate.setVisibility(GONE);
                mGnSearchBgLayout.setVisibility(VISIBLE);
                
                setupSearchViewUnfoldAnim();
                mGnSearchBgLayout.startAnimation(mSearchViewUnfoldAnim);
            } else if (animation == mSearchViewUnfoldAnim) {
                mSearchPlate.setVisibility(VISIBLE);
                mGnSearchBgLayout.setVisibility(VISIBLE);
                if (mIsSearchSubmitMode && mGnSearchGoButton != null) {
                    mGnSearchGoButton.setVisibility(VISIBLE);
    ...
    private boolean isGioneeWidget3Support() {
        return true;///*SystemProperties.get("ro.gn.widget3.support").equals("yes");*/
    }
    // Gionee <zhangxx><2013-05-09> add for CR00811583 end
}
