 ...
 */
public class CyeeAutoCompleteTextView extends CyeeEditText implements Filter.FilterListener {
    ...
    private final int mHintResource;
    ...
    private final Drawable mListSelector;

    public CyeeAutoCompleteTextView(Context context) {
        this(context, null);
    }

    public CyeeAutoCompleteTextView(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.autoCompleteTextViewStyle);
    }

    public CyeeAutoCompleteTextView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a =
            context.obtainStyledAttributes(
                attrs, R.styleable.CyeeAutoCompleteTextView, defStyle, 0);

        mThreshold = a.getInt(
                R.styleable.CyeeAutoCompleteTextView_cyeecompletionThreshold, 2);
        mListSelector = a.getDrawable(R.styleable.CyeeAutoCompleteTextView_cyeedropDownSelector);
        mPopup.setListSelector(mListSelector);
        mPopup.setVerticalOffset((int)
                a.getDimension(R.styleable.CyeeAutoCompleteTextView_cyeedropDownVerticalOffset, 0.0f));
        mPopup.setHorizontalOffset((int)
                a.getDimension(R.styleable.CyeeAutoCompleteTextView_cyeedropDownHorizontalOffset, 0.0f));
        
        // Get the anchor's id now, but the view won't be ready, so wait to actually get the
        // view and store it in mDropDownAnchorView lazily in getDropDownAnchorView later.
        // Defaults to NO_ID, in which case the getDropDownAnchorView method will simply return
        // this TextView, as a default anchoring point.
        mDropDownAnchorId = a.getResourceId(R.styleable.CyeeAutoCompleteTextView_cyeedropDownAnchor,
                View.NO_ID);
        
        // For dropdown width, the developer can specify a specific width, or MATCH_PARENT
        // (for full screen width) or WRAP_CONTENT (to match the width of the anchored view).
        mPopup.setWidth(a.getLayoutDimension(
                R.styleable.CyeeAutoCompleteTextView_cyeedropDownWidth,
                ViewGroup.LayoutParams.WRAP_CONTENT));
        mPopup.setHeight(a.getLayoutDimension(
                R.styleable.CyeeAutoCompleteTextView_cyeedropDownHeight,
                ViewGroup.LayoutParams.WRAP_CONTENT));

        mHintResource = a.getResourceId(R.styleable.CyeeAutoCompleteTextView_cyeecompletionHintView,
                R.layout.cyee_simple_dropdown_hint);
        
        mPopup.setOnItemClickListener(new DropDownItemClickListener());
        setCompletionHint(a.getText(R.styleable.CyeeAutoCompleteTextView_cyeecompletionHint));
        ...
        if (ChameleonColorManager.isNeedChangeColor(context)) {
            ...
            changeListSelector(mListSelector);
        }
    }
    ...
    private void changeListSelector(Drawable selector) {
        if (selector != null) {
            if (selector instanceof RippleDrawable) {
                ((RippleDrawable) selector).setColor(ColorStateList
                        .valueOf(ChameleonColorManager.getContentColorThirdlyOnBackgroud_C3()));
            }
        }
    }
        ...
        if (hint != null) {
            if (mHintView == null) {
                final TextView hintView = (TextView) LayoutInflater.from(getContext()).inflate(
                        mHintResource, null).findViewById(com.cyee.internal.R.id.cyee_text1);
    ...
    public void setDropDownHorizontalOffset(int offset) {
        ...
        // Gionee <lihq> <2013-8-26> modify for CR00871548 begin
        //setDropDownWidth(500);
        // Gionee <lihq> <2013-8-26> modify for CR00871548 end
    }
     ...
     * @see CyeeAutoCompleteTextView#setOnDismissListener(OnDismissListener)
     */
