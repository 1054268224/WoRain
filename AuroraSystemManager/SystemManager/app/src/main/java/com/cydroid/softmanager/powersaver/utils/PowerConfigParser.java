...
public class PowerConfigParser {
    ...
    public static String projectName = SystemProperties.get("ro.cy.common.mainboard.prop");
    public static String custom = SystemProperties.get("ro.cy.custom");
...
//                        Debug.log(DEBUG, TAG, "PowerConfig-->getProjectConfig(), START_TAG");
            ...
            Log.e(true, TAG, "PowerConfig-->getProjectConfig() throw exception, new PowerConfig default", e);
...
//            Log.d(TAG, "read config file " + CONFIG_FILE_PATH);
            if (file.exists()) {
                return new FileInputStream(file);
            } else {
                Log.d(TAG, "config file " + CONFIG_FILE_PATH + " not exist");
            }
        } catch (Exception e) {
            Log.d(DEBUG, TAG, "PowerConfig-->getConfigStream(), read CONFIG_FILE wrong or some problems");
        }
        InputStream is = null;
        try {
            Log.d(DEBUG, TAG, "PowerConfig-->getConfigStream(), open config.xml");
            is = context.getAssets().open("config.xml");
        } catch (Exception e) {
            Log.d(DEBUG, TAG, "PowerConfig-->getConfigStream(),open asset/config.xml error");
        }
