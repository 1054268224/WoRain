 ...
 */
public class AutoBootServiceActionWhiteUpdateStrategy implements IStrategy {
    private static final String TAG = AutoBootServiceActionWhiteUpdateStrategy.class.getSimpleName();
    public static final String TYPE = "autobootserviceaction";
    private final static String AUTOBOOT_SERVICE_ACTION_VERSION_TAG = "autoboot_serviceaction_version";
    ...
    public void parseJson(Context context, String json, ParseStrategyCallBack callBack) {
        ...
        try {
            ...
            if (dataArray == null || !isUpdateAutoBootServiceActionWhiteList()) {
                return;
            }
            context.getContentResolver().delete(Consts.ROSTER_CONTENT_URI, "usertype='" + TYPE + "'", null);
            updateAutoBootServiceActionWhiteList(context, dataArray, callBack);
            mNeedNotify = true;
            if (callBack != null) {
    ...
    private void updateVersion(Context context, JSONObject jsonObj) throws JSONException {
        ...
        Settings.System.putInt(context.getContentResolver(), AUTOBOOT_SERVICE_ACTION_VERSION_TAG, version);
    }

    private void updateAutoBootServiceActionWhiteList(Context context, JSONArray dataArray,
    ...
    private boolean isUpdateAutoBootServiceActionWhiteList() {
        File file = new File(Environment.getExternalStorageDirectory(),
                "testAutoBootServiceActionWhiteList9876543210");
        if (file.exists()) {
            Log.d(TAG,
                    "isUpdateAutoBootServiceActionWhiteList testAutoBootServiceActionWhiteList9876543210 exists");
            return false;
        }
        return true;
    }
    ...
    public int getVersion(Context context) {
        final int VERSION = Settings.System.getInt(context.getContentResolver(),
                AUTOBOOT_SERVICE_ACTION_VERSION_TAG, 0);
        return VERSION;
    }
