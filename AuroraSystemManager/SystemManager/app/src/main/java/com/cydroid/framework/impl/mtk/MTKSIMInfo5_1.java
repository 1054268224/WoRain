...
import com.cydroid.framework.ISIMInfo;
...
public class MTKSIMInfo5_1 implements ISIMInfo {
    ...
    private static final Class<?> mClass = FrameworkUtility.createClass(CLASS_NAME);
    private static final Class<?> mSubinfoClass = FrameworkUtility.createClass(SUBINFO_CLASS_NAME);

    public MTKSIMInfo5_1() {
    }
    ...
    public SIMInfo getSIMInfoBySlot(Context ctx, int cardSlot) {
        // Gionee: mengdw <2015-10-29> modify for CR01565490 begin
        try {
            List<SIMInfo> SubInfoRecords = getInsertedSIMList(ctx);
            for (SIMInfo record : SubInfoRecords) {
                if(record != null) {
                    Log.d("MTKSIMInfo5_1","getSIMInfoBySlot record!=null");
                } else {
                    Log.d("MTKSIMInfo5_1","getSIMInfoBySlot record==null");
                }
                if (record !=null && record.mSlot == cardSlot) {
                    return record;
                }
            }
        } catch (Exception e) {
            Log.d("MTKSIMInfo5_1", "getSIMInfoBySlot exception =" + e.toString());
        }
        // Gionee: mengdw <2015-10-29> modify for CR01565490 end
        return null;
    }
