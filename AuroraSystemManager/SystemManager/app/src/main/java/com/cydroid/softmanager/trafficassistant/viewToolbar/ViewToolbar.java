...
public abstract class ViewToolbar {
    ...
    private static final int TOOLBAR_ITEM_PADDING_LEFT_AND_RIGHT = 6;// 12;
    ...
    private final int mCenterDrawableResId;
    private final Drawable mRightDrawable;
    private final Drawable mSingleDrawable;
    private final Drawable mArrowAboveDrawable;
    private final Drawable mArrowBelowDrawable;

    private final int mStatusBarHeight;
    private final int mToolbarPositionArrowWidth;
    private final int mToolbarPositionArrowHeight;
    ...
    public ViewToolbar(Context context, int left, int top) {
        // this.mHostView = context;
        this.mContext = context;// mHostView.getContext();
        mLeft = left;
        mTop = top;
        Log.d("Gaoj", "mLeft = " + mLeft);
        Log.d("Gaoj", "mTop = " + mTop);

        // initial resources
        Resources resources = context.getResources();// mHostView.getResources();
        ...
        mToolbarView = mLayoutInflater.inflate(R.layout.trafficassistant_text_toolbar, null);
        mToolbarGroup = (ViewGroup) mToolbarView.findViewById(R.id.gn_toolbar_group);
        // Gionee CR00311071 jipengfei 20110730: comment out this line, it has
        // nonsense
    ...
    public void hide() {
        if (mShowing) {
            try {
                // Gionee CR00311071 jipengfei 20110730: comment out this line,
                // it has nonsense
                // mToolbarGroup.setPadding(2, 2, 2, 2);
    ...
    private void prepare(int screenX, int screenY, int cursorLineHeight, boolean selected) {
        ...
        // Gionee CR00311071 jipengfei 20110730: modify the calculations of
        // paddingLeft start
        paddingLeft = Math.max(TOOLBAR_ARROW_OFFSET_TO_EDGE + TOOLBAR_POSITION_OFFSET_TO_SCREEN_LEFT,
                paddingLeft);
        paddingLeft = Math.min(mToolbarGroup.getMeasuredWidth() - mToolbarPositionArrowWidth
                - TOOLBAR_ARROW_OFFSET_TO_EDGE - TOOLBAR_POSITION_OFFSET_TO_SCREEN_RIGHT, paddingLeft);
        if (aboveCursor) {
            mToolbarPositionArrowView.setImageDrawable(mArrowBelowDrawable);
            // mToolbarGroup.setPadding(2, 2, 2, 2);
            mToolbarPositionArrowView.setPadding(paddingLeft, mToolbarGroup.getMeasuredHeight()
                    - mArrowBelowDrawable.getIntrinsicHeight(), 0, 0);
        } else {
        ...
        if (childCount >= 2) {
            for (int i = 0; i < childCount; i++) {
                ...
                if (i == 0) {
                    ...
                    view.setPadding(mToolbarItemPaddingLeftAndRight * 2 + 1, 0,
                            mToolbarItemPaddingLeftAndRight, mToolbarItemPaddingBottom);
                } else if (i == childCount - 1) {
                    view.setBackgroundDrawable(mRightDrawable);
                    view.setPadding(mToolbarItemPaddingLeftAndRight, 0, mToolbarItemPaddingLeftAndRight * 2,
                            mToolbarItemPaddingBottom);
                } else {
                    view.setBackgroundResource(mCenterDrawableResId);
                    view.setPadding(mToolbarItemPaddingLeftAndRight, 0, mToolbarItemPaddingLeftAndRight + 1,
                            mToolbarItemPaddingBottom);
                }
            }
            ...
            view.setPadding(mToolbarItemPaddingLeftAndRight * 2, mToolbarItemPaddingBottom * 2,
                    mToolbarItemPaddingLeftAndRight * 2, mToolbarItemPaddingBottom);
        }
    }
    ...
    private boolean calculatePosition(int screenX, int screenY, int cursorLineHeight, boolean selected) {
        ...
        // calculate x
        int x;
        int px = screenX - mLeft;// mHostView.getRootView().getScrollX();
        ...
        // calculate y
        int y;
        int py = screenY - mTop;// mHostView.getRootView().getScrollY();
        ...
        /*
         * boolean aboveCursor = true; // calculate x int px = screenX -
         * mHostView.getRootView().getScrollX(); int half =
         * mToolbarGroup.getMeasuredWidth() / 2; int displayWidth =
         * mWindowManager.getDefaultDisplay().getWidth(); //Gionee CR00311071
         * jipengfei 20110730: modify the calculations of mPositionX start int
         * leftEdge = px - half; int rightEdge = px + half; if(leftEdge <
         * -TOOLBAR_POSITION_OFFSET_TO_SCREEN_LEFT){ mPositionX =
         * -TOOLBAR_POSITION_OFFSET_TO_SCREEN_LEFT; }else if(rightEdge >
         * displayWidth + TOOLBAR_POSITION_OFFSET_TO_SCREEN_RIGHT){ mPositionX =
         * displayWidth + TOOLBAR_POSITION_OFFSET_TO_SCREEN_RIGHT -
         * mToolbarGroup.getMeasuredWidth(); }else{ mPositionX = px - half; }
         * 
         * // calculate y int py = screenY -
         * mHostView.getRootView().getScrollY(); int th =
         * mToolbarGroup.getMeasuredHeight() + mToolbarPositionArrowHeight; int
         * lh = cursorLineHeight; if (py - th - lh < mStatusBarHeight) {
         * mPositionY = py + lh + (selected ? mToleranceTouch : 0); aboveCursor
         * = false; } else { mPositionY = py - th - (selected ? mToleranceTouch
         * : 0); aboveCursor = true; } //Gionee CR00311071 jipengfei 20110730:
         * modify the calculations of mPositionX end return aboveCursor;
         */
    }
    ...
    private void calculateTolerance() {
        ...
        mToolbarItemPaddingLeftAndRight = Math.round(TOOLBAR_ITEM_PADDING_LEFT_AND_RIGHT * ratio) / 2;
