...
public class CyeeProvider extends ContentProvider {

    private static final String TAG = "CyeeProvider";
    private static final boolean LOCAL_LOGV = true;
    
    private CyeeDBHelper mOpenHelper = null;
    ...
    public boolean onCreate() {
        if (mOpenHelper == null) {
            mOpenHelper = new CyeeDBHelper(getContext());
        }
    ...
    public Bundle call(String method, String name, Bundle args) {
        if (CyeeSettings.CALL_METHOD_GET_CONFIG.equals(method)) {
            return lookupValue(name);
        } else if (CyeeSettings.CALL_METHOD_SET_CONFIG.equals(method)) {
            return putString(name, args.getString(CyeeSettings.VALUE));
        } else {
            return null;
        }
    }
    ...
    private void sendNotify(Uri uri, int userHandle) {
        ...
        if (table.equals(CyeeSettings.TABLE_CONFIG)) {
            property = CyeeSettings.CYEE_PROP_SETTING_VERSION;    // this one is global
    ...
    private Bundle lookupValue(String name) {
        // Log.d(TAG, "CyeeProvider lookupValue get value from name " + name);
        if(!mConfigInited) {
            SQLiteDatabase dbw = gnGetWritableDatabase();
            mOpenHelper.updateDBValue(dbw, false);
            mConfigInited = true;
            Log.d(TAG, "CyeeProvider lookupValue mConfigInited ");
        }
        
        SQLiteDatabase db = gnGetReadableDatabase();
        Cursor cursor = null;
        try {
            cursor = db.query(CyeeSettings.TABLE_CONFIG, COLUMN_VALUE, "name=?", new String[]{ name },
                              null, null, null, null);
            if (cursor != null && cursor.getCount() == 1) {
                cursor.moveToFirst();
                
                Bundle bundle = new Bundle();
                bundle.putString(CyeeSettings.VALUE, cursor.getString(0));
                return bundle;
            }
        } catch (SQLiteException e) {
            Log.w(TAG, "cyeesettings lookup error", e);
            return null;
        } finally {
    ...
    public Bundle putString(String name, String value) {
        Log.d(TAG, "CyeeSettings put string name = " + name + " , value = " + value);

        SQLiteDatabase db = gnGetWritableDatabase();
        
        final ContentValues values = new ContentValues();
        values.put(CyeeSettings.NAME, name);
        values.put(CyeeSettings.VALUE, value);
        
        long count = db.update(CyeeSettings.TABLE_CONFIG, values, "name=?", new String[] { name });
        Bundle bundle = new Bundle();
        bundle.putBoolean(CyeeSettings.VALUE, (count > 0) ? true : false);
        try {
            if (count == 0 && name.endsWith(".miss_infos")) {                
                count = db.insert(CyeeSettings.TABLE_CONFIG, null, values);
                bundle.putBoolean(CyeeSettings.VALUE, (count > 0) ? true : false);
            }
        } catch (Exception e) {
            Log.w(TAG, "putString", e);
        }
        
        sendNotify(CyeeSettings.getUriFor(name), UserHandle.getCallingUserId());
        return bundle;
    }
    ...
    private String getDefaultRingtoneFile(int type) {
        ...
        String filename = SystemProperties.get(DEFAULT_RINGTONE_PROPERTY_PREFIX + CyeeSettings.RINGTONE2);

        switch (type) {
            //case RingtoneManager.TYPE_RINGTONE2:
            case TYPE_RINGTONE2:
                filename = SystemProperties.get(DEFAULT_RINGTONE_PROPERTY_PREFIX + CyeeSettings.RINGTONE2);
                break;
            //case RingtoneManager.TYPE_MMS:
			case TYPE_MMS:
                filename = SystemProperties.get(DEFAULT_RINGTONE_PROPERTY_PREFIX + CyeeSettings.MMS);
                break;
            //case RingtoneManager.TYPE_MMS2:
			case TYPE_MMS2:
                filename = SystemProperties.get(DEFAULT_RINGTONE_PROPERTY_PREFIX + CyeeSettings.MMS2);
                break;
            default:
                break;
        }
    ...
    private void setDefaultRingtone(int type, String filename) {
        ...
        while (cursor.moveToNext()) {
            ...
            CyeeSettings.putString(getContext().getContentResolver(), name, ContentUris
