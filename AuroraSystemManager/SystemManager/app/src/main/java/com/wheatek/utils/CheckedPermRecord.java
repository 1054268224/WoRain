...
public class CheckedPermRecord implements Parcelable {
    ...
    public boolean isSystemApp() {
        if (mUid >= 0) {
            return (mUid < android.os.Process.FIRST_APPLICATION_UID);
        } else {
            return false;
        }
    }
    ...
    public void removePkgName(String pkgName) {
        if (pkgName == null || mPackageName == null) {
            return;
        }
        String pkgremove = pkgName + ";";
        mPackageName = mPackageName.replaceAll(pkgremove, "");
    }
    ...
    public String toString() {
        return "CheckedPermRecord {" + ", mPackageName: " + mPackageName
                + ", mUid: " + mUid + ", mStatus: " + mStatus + "}";
    }
    ...
    public static final Creator<CheckedPermRecord> CREATOR = new Creator<CheckedPermRecord>() {
        @Override
