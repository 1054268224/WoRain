 ...
 */
public class CyeeExpandableListView extends CyeeListView {
    ...
    private CyeeExpandableListConnector mConnector;
    ...
    private final int mGroupDividerColor;
    
    private boolean isAutoIndented = true;
    
    // Bounds of the indicator to be drawn
    private final Rect mIndicatorRect = new Rect();

    public CyeeExpandableListView(Context context) {
        this(context, null);
    }

    public CyeeExpandableListView(Context context, AttributeSet attrs) {
        this(context, attrs, com.cyee.internal.R.attr.cyeeExpandableListViewStyle);
    }

    public CyeeExpandableListView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        mContext = context;
        
        TypedArray a =
            context.obtainStyledAttributes(attrs, com.cyee.internal.R.styleable.CyeeExpandableListView, defStyle,
                    0);

		mGroupIndicator = a
				.getDrawable(com.cyee.internal.R.styleable.CyeeExpandableListView_cyeegroupIndicator);

		// gionee maxw add begin
		if (mGroupIndicator instanceof StateListDrawable) {
			indicatorSetColorFilter((StateListDrawable) mGroupIndicator);
		}
		// gionee maxw add end
        mChildIndicator = a
                .getDrawable(com.cyee.internal.R.styleable.CyeeExpandableListView_cyeechildIndicator);
        mIndicatorLeft = a
                .getDimensionPixelSize(com.cyee.internal.R.styleable.CyeeExpandableListView_cyeeindicatorLeft, 0);
        mIndicatorRight = a
                .getDimensionPixelSize(com.cyee.internal.R.styleable.CyeeExpandableListView_cyeeindicatorRight, 0);
        if (mIndicatorRight == 0 && mGroupIndicator != null) {
            mIndicatorRight = mIndicatorLeft + mGroupIndicator.getIntrinsicWidth();
        }
        mChildIndicatorLeft = a.getDimensionPixelSize(
                com.cyee.internal.R.styleable.CyeeExpandableListView_cyeechildIndicatorLeft, CHILD_INDICATOR_INHERIT);
        mChildIndicatorRight = a.getDimensionPixelSize(
                com.cyee.internal.R.styleable.CyeeExpandableListView_cyeechildIndicatorRight, CHILD_INDICATOR_INHERIT);
        mChildDivider = a.getDrawable(com.cyee.internal.R.styleable.CyeeExpandableListView_cyeechildDivider);
        
        mGroupDividerColor = a.getColor(com.cyee.internal.R.styleable.CyeeExpandableListView_cyeeGroupDivider, 0);
        
        Drawable draw = context.getResources().getDrawable(com.cyee.internal.R.drawable.cyee_transparent);
        setChildDivider(draw);
        // Gionee <gaoj> <2013-08-27> added for CR00833450 begin
        mItemPadding = (int)mContext.getResources().getDimension(com.cyee.internal.R.dimen.cyee_expandablelistview_item_padding);
        Log.d("CyeeExpandListView", "mItemPadding = "+mItemPadding);
        // Gionee <gaoj> <2013-08-27> added for CR00833450 end
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        // gionee maxw add begin
        // draw the group divider if need
        ...
        int lastItemType = ~(CyeeExpandableListPosition.CHILD | CyeeExpandableListPosition.GROUP);
        ...
        for (int i = 0, childFlPos = getFirstVisiblePosition() - headerViewsCount; i < childCount;
            ...
            if (mAnimatorEnabled) {
                if (pos.isExpanded()
                        && pos.position.type == CyeeExpandableListPosition.GROUP) {
                    mL = item.getLeft();
                    mR = item.getRight();
                }
                if (pos.isExpanded()
                        && pos.position.type == CyeeExpandableListPosition.CHILD) {
                    item.setLeft(mL + mItemPadding);
                    item.setRight(mR - mItemPadding);
                }
            }
            // Gionee <gaoj> <2013-08-27> added for CR00833450 end
            ...
            if (pos.position.type != lastItemType) {
                if (pos.position.type == CyeeExpandableListPosition.CHILD) {
                    ...
                    // Gionee <lihq> <2013-12-25> add for CR00873172 begin
                    if (mAnimatorEnabled) {
                        indicatorRect.left += (mPaddingLeft + mItemPadding);
                        indicatorRect.right += (mPaddingLeft + mItemPadding);
                    }
                    // Gionee <lihq> <2013-12-25> add for CR00873172 end
                } else {
                    indicatorRect.left = mIndicatorLeft;
                    indicatorRect.right = mIndicatorRight;
                }
                // Gionee <lihq> <2013-12-25> modify for CR00873172 begin
                // Gionee <gaoj> <2013-08-27> added for CR00833450 begin
                /*
                if (mAnimatorEnabled) {
                    indicatorRect.left += (mPaddingLeft + mItemPadding);
                ...
                */
                ...
                indicatorRect.left += mPaddingLeft;
                indicatorRect.right += mPaddingLeft;
                // Gionee <lihq> <2013-12-25> modify for CR00873172 end
	...
	private void drawGroupDivider(Canvas canvas) {
        ...
        paint.setColor(mGroupDividerColor);
        int deviderHeightId = com.cyee.internal.R.dimen.cyee_divider_height;
        paint.setStrokeWidth(mContext.getResources().getDimension(deviderHeightId));

        // changeColor
        if (ChameleonColorManager.isNeedChangeColor(mContext)) {
        	paint.setColor(ChameleonColorManager
        ...
        if (ChameleonColorManager.getInstance().getCyeeThemeType(mContext) == ChameleonColorManager.CyeeThemeType.GLOBAL_THEME) {
            paint.setAlpha((int) (255 * GROUP_DIVIDER_ALPHA));            
        }
        for (int i = 0, childFlPos = getFirstVisiblePosition() - headerViewsCount; i < childCount; i++, childFlPos++) {
            ...
            if (pos.position.type == CyeeExpandableListPosition.GROUP && needShowDivider(childFlPos)) {
                // int left = mGroupIndicator == null? 0:
                // mGroupIndicator.getIntrinsicWidth()+mIndicatorLeft*2;
                canvas.drawLine(0, top, getRight(), top, paint);
            }
            if (pos.position.type == CyeeExpandableListPosition.CHILD && isAutoIndented) {
                int left = mGroupIndicator == null ? 0 : mGroupIndicator.getIntrinsicWidth() + mIndicatorLeft
                        * 2;
                item.setPaddingRelative(left, 0, 0, 0);
            }

            if (i == childCount - headerViewsCount - getFooterViewsCount() - 1 && needShowDivider(childFlPos)) {
                canvas.drawLine(0, item.getBottom(), getRight(), item.getBottom(), paint);
            }
        }
	}
    ...
    private Drawable getIndicator(PositionMetadata pos) {
        ...
        if (pos.position.type == CyeeExpandableListPosition.GROUP) {
            ...
            // Gionee <gaoj> <2013-08-27> added for CR00833450 begin
            if (mAnimatorEnabled) {
                if (mAdapter.getChildrenCount(pos.position.groupPos) == 0) {
                    indicator = mContext.getResources().getDrawable(
                            com.cyee.internal.R.drawable.cyee_expander_open_dark);
                }
            }
            // Gionee <gaoj> <2013-08-27> added for CR00833450 end
        } else {
	...
	private void indicatorSetColorFilter(StateListDrawable stateListDrawble) {
		for (int index = 0; index < stateListDrawble.getStateCount(); index++) {
			...
			boolean isExpanded = isStateExpanded(stateListDrawble.getStateSet(index));
			changeColorGroupIndicator(drawable, isExpanded);
		}
	}
	
    private void changeColorGroupIndicator(Drawable drawable, boolean isExpanded) {
        int color = 0xff00a3e4;
        // gionee maxw add begin
        if(isExpanded) {
        ...
        // gionee maxw add end
        if (ChameleonColorManager.isNeedChangeColor(mContext)) {
            color = isExpanded ? ChameleonColorManager.getAccentColor_G1() : ChameleonColorManager
	...
	private boolean isStateExpanded(int[] myDrawableState) {
		for (int index = 0; index < myDrawableState.length; index++) {
			if (myDrawableState[index] == android.R.attr.state_expanded) {
				return true;
			}
		}
		return false;
    ...
    /**
     * set if auto indent or not
     * 
     * @param isAutoIndent
     */
    public void setIsAutoIndent(boolean isAutoIndent) {
        this.isAutoIndented = isAutoIndent;
    }

	/**
	 * Sets the drawable that will be drawn adjacent to every child in the list. This will be drawn using the
	 * same height as the normal divider ({@link #setDivider(Drawable)}) or if it does not have an intrinsic
    ...
    void drawDivider(Canvas canvas, Rect bounds, int childIndex) {
        ...
        if (flatListPosition >= 0) {
            ...
            if ((pos.position.type == CyeeExpandableListPosition.CHILD) || (pos.isExpanded() &&
                    pos.groupMetadata.lastChildFlPos != pos.groupMetadata.flPos)) {
    ...
    public void setAdapter(ExpandableListAdapter adapter) {
        ...
        if (adapter != null) {
            ...
            mConnector = new CyeeExpandableListConnector(mContext, adapter);
        } else {
            mConnector = null;
        }
    ...
    boolean handleItemClick(View v, int position, long id) {
        ...
        boolean returnValue;
        if (posMetadata.position.type == CyeeExpandableListPosition.GROUP) {
            ...
            if (posMetadata.isExpanded()) {
                Log.v("CyeeExpandListView", "handleItemClick setExpandAnimFlg(false)");
                // Gionee <gaoj> <2013-08-27> modify for CR00833450 begin
                if (mAnimatorEnabled) {
                    if (mAdapter.getChildrenCount(posMetadata.position.groupPos) == 0) {
                        return false;
                    } else {
                    ...
                    if (mOnGroupCollapseListener != null) {
                        mOnGroupCollapseListener
                                .onGroupCollapse(posMetadata.position.groupPos);
                    }
                    posMetadata.recycle();
                }
                // Gionee <gaoj> <2013-08-27> modify for CR00833450 end
            } else {
                Log.v("CyeeExpandListView", "handleItemClick setExpandAnimFlg(true)");
                ...
                // Gionee <gaoj> <2013-08-27> added for CR00833450 begin
                if (mAnimatorEnabled) {
                    mConnector.setGroupPos(groupPos);
                }
                // Gionee <gaoj> <2013-08-27> added for CR00833450 end
    ...
    public boolean expandGroup(int groupPos, boolean animate) {
        CyeeExpandableListPosition elGroupPos = CyeeExpandableListPosition.obtain(
                CyeeExpandableListPosition.GROUP, groupPos, -1, -1);
    ...
    public interface OnGroupClickListener {
         ...
         */
        boolean onGroupClick(CyeeExpandableListView parent, View v, int groupPosition,
                long id);
    }
    ...
    public interface OnChildClickListener {
         ...
         * @return True if the click was handled
         */
        boolean onChildClick(CyeeExpandableListView parent, View v, int groupPosition,
    ...
    public int getFlatListPosition(long packedPosition) {
        CyeeExpandableListPosition elPackedPos = CyeeExpandableListPosition
    ...
    public void setSelectedGroup(int groupPosition) {
        CyeeExpandableListPosition elGroupPos = CyeeExpandableListPosition
    ...
    public boolean setSelectedChild(int groupPosition, int childPosition, boolean shouldExpandGroup) {
        CyeeExpandableListPosition elChildPos = CyeeExpandableListPosition.obtainChildPosition(
    ...
    @Override
    ContextMenuInfo cyeeCreateContextMenuInfo(View view, int flatListPosition, long id) {
        if (isHeaderOrFooterPosition(flatListPosition)) {
        ...
        final int adjustedPosition = getFlatPositionForConnector(flatListPosition);
        PositionMetadata pm = mConnector.getUnflattenedPos(adjustedPosition);
        CyeeExpandableListPosition pos = pm.position;
     ...
     *            returned.
     */
    private long getChildOrGroupId(CyeeExpandableListPosition position) {
        if (position.type == CyeeExpandableListPosition.CHILD) {
    ...
    static class SavedState extends BaseSavedState {
        ArrayList<CyeeExpandableListConnector.GroupMetadata> expandedGroupMetadataList;
        ...
        private SavedState(Parcel in) {
            super(in);
            expandedGroupMetadataList = new ArrayList<CyeeExpandableListConnector.GroupMetadata>();
            in.readList(expandedGroupMetadataList, CyeeExpandableListConnector.class.getClassLoader());
        }
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(CyeeExpandableListView.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(CyeeExpandableListView.class.getName());
    }
    ...
    private void startCollapseGroupAnimation(final PositionMetadata posMetadata) {
        Log.v("CyeeExpandListView", "startCollapseGroupAnimation");
    ...
    public void collapseAnimation(final PositionMetadata posMetadata) {
        ...
        Log.v("CyeeExpandListView", "getChildCount() = " + getChildCount());
        View vi = null;
        ...
        for (int i = 0, childFlPos = getFirstVisiblePosition() - headerViewsCount; i < childCount;
            ...
            if (pos.isExpanded() && pos.position.groupPos == posMetadata.position.groupPos
                    && pos.position.type == CyeeExpandableListPosition.CHILD) {
                vi = getChildAt(i);
                Log.v("CyeeExpandListView", "vi = " + vi);
                views.add(vi);
            }
        }
        ...
        if (!views.isEmpty()) {
            ...
            Log.v("CyeeExpandListView", "views.size() = " + views.size());
            View view = null;
            ...
            animatorSet.addListener(new AnimatorListener() {
                ...
                public void onAnimationEnd(Animator arg0) {
                    Log.v("CyeeExpandListView", "animateDismiss onAnimationEnd");
    ...
    public int[] mHeight = { 128, 118, 98, 78, 58, 38, 18 };

    private Animator createcollapseAnimatorForView(final View view, int pos) {
        final ViewGroup.LayoutParams lp = view.getLayoutParams();
        final int originalHeight = view.getHeight();

        int height = (pos < 7 ? originalHeight - mHeight[pos] : 0);
        Log.v("CyeeExpandListView", "close  heght = " + height);
        ValueAnimator animator = ValueAnimator.ofInt(originalHeight, height);
        ...
        animator.addListener(new AnimatorListener() {
            ...
            @Override
            public void onAnimationEnd(Animator arg0) {
                Log.v("CyeeExpandListView",
                        "createcollapseAnimatorForView onAnimationEnd");
                lp.height = 0;
                view.setAlpha(1.0f);
        ...
        animator.addUpdateListener(new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                Log.v("CyeeExpandListView",
                        "createcollapseAnimatorForView onAnimationUpdate lp.height = "
                                + lp.height);
    ...
    private void startCollapseGroup(PositionMetadata posMetadata) {
        Log.v("CyeeExpandListView", "startCollapseGroup");
    ...
    private boolean mAnimatorEnabled = false;

    public void setAnimatorEnabled(boolean enabled) {
        mAnimatorEnabled = enabled;
        if (mConnector != null) {
            mConnector.setConnectorAnimatorEnabled(enabled);
        }
        if (mAnimatorEnabled) {
            setSelector(com.cyee.internal.R.drawable.cyee_transparent);
            setDivider(null);
        }
        Log.v("CyeeExpandListView", "setAnimatorEnabled mAnimatorEnabled = "
                + mAnimatorEnabled);
    }

    public boolean getAnimatorEnabled() {
        Log.v("CyeeExpandListView", "getAnimatorEnabled mAnimatorEnabled = "
                + mAnimatorEnabled);
        return mAnimatorEnabled;
    }
    // Gionee <gaoj> <2013-08-27> modify for CR00833450 end
}
