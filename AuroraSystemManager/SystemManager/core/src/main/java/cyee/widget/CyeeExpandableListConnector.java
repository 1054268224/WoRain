 ...
 * expects.
 */
class CyeeExpandableListConnector extends BaseAdapter implements Filterable {
    /**
     ...
     */
    public CyeeExpandableListConnector(ExpandableListAdapter expandableListAdapter) {
    ...
    PositionMetadata getUnflattenedPos(final int flPos) {
        ...
        if (numExpGroups == 0) {
             ...
             * same as its flPos
             */
            return PositionMetadata.obtain(flPos, CyeeExpandableListPosition.GROUP, flPos,
                    -1, null, 0);
        }
        ...
        while (leftExpGroupIndex <= rightExpGroupIndex) {
            ...
            if (flPos > midExpGm.lastChildFlPos) {
                 ...
                 */
                return PositionMetadata.obtain(flPos, CyeeExpandableListPosition.GROUP,
                ...
                return PositionMetadata.obtain(flPos, CyeeExpandableListPosition.CHILD,
        ...
        return PositionMetadata.obtain(flPos, CyeeExpandableListPosition.GROUP, groupPos, -1,
                null, insertPosition);
    }
     ...
     */
    PositionMetadata getFlattenedPos(final CyeeExpandableListPosition pos) {
        final ArrayList<GroupMetadata> egml = mExpGroupMetadataList;
        final int numExpGroups = egml.size();
        ...
        while (leftExpGroupIndex <= rightExpGroupIndex) {
            ...
            if (pos.groupPos > midExpGm.gPos) {
                ...
                if (pos.type == CyeeExpandableListPosition.GROUP) {
                ...
                } else if (pos.type == CyeeExpandableListPosition.CHILD) {
    ...
    public boolean isEnabled(int flatListPos) {
        ...
        final CyeeExpandableListPosition pos = metadata.position;
        
        boolean retValue;
        if (pos.type == CyeeExpandableListPosition.CHILD) {
    ...
    public Object getItem(int flatListPos) {
        ...
        } else if (posMetadata.position.type == CyeeExpandableListPosition.CHILD) {
    ...
    public View getView(int flatListPos, View convertView, ViewGroup parent) {
        ...
        if (posMetadata.position.type == CyeeExpandableListPosition.GROUP) {
			...
			// drop this function: modify mAnimatorEnabled always false, never go here
            if (mAnimatorEnabled) {
                int childcount = mExpandableListAdapter
                        .getChildrenCount(posMetadata.position.groupPos);
                if (posMetadata.isExpanded() && childcount > 0) {
                    retValue.setBackgroundResource(com.cyee.internal.R.drawable.cyee_list_selector_light);
                } else {
                    if (childcount == 0) {
                        retValue.setBackgroundResource(com.cyee.internal.R.drawable.cyee_expandlist_group_normal_bg);
                    } else {
                        retValue.setBackgroundResource(com.cyee.internal.R.drawable.cyee_expandlist_group_bg_light);
                    }
                }
            }
            ...
            // gionee maxw add begin;
            retValue.setMinimumHeight(mChildItemHeight);
            // gionee maxw add end;
            ...
            if (mAnimatorEnabled) {
                ...
                if (mIsExpandGroup
                        && mGroupPos == posMetadata.position.groupPos) {
...
// gionee maxw add begin
	...
	private void setTextColor(TextView tv, boolean isExpanded) {
		...
		int[] initColors = colorStateList.getColors();
		int[][] initStates = colorStateList.getStates();

		if (isExpanded) {
			if (!isModified(initStates)) {
				// need to modify the color

				int[][] newStates = new int[initStates.length + 2][];
				int[] newColors = new int[initColors.length + 2];

				System.arraycopy(initStates, 0, newStates, 2, initStates.length);
				newStates[0] = new int[] {};
				newStates[1] = new int[] { -1 };

				System.arraycopy(initColors, 0, newColors, 2, initColors.length);
				newColors[0] = 0xff00a3e4;
				newColors[1] = 0xff00a3e4;
				
				if (ChameleonColorManager.isNeedChangeColor(tv.getContext())) {
				    newColors[0] = ChameleonColorManager.getAccentColor_G1();
				}

				ColorStateList newColorStateList = new ColorStateList(
						newStates, newColors);
				 tv.setTextColor(newColorStateList);
			}
		} else {
			if (isModified(initStates)) {
				// need to remove the modify
				
				int[][] newStates = new int[initStates.length - 2][];
				int[] newColors = new int[initColors.length - 2];

				System.arraycopy(initStates, 2, newStates, 0, newStates.length);
				System.arraycopy(initColors, 2, newColors, 0, newColors.length);
	...
	private boolean isModified(int[][] initStates) {
		...
		for(int[] item : initStates) {
			for(int state : item) {
				// -1 is the value of flag that modified state;
				if(state == -1) {
					isModified = true;
					break;
				}
    ...
    static public class PositionMetadata {
        ...
        static PositionMetadata obtain(int flatListPos, int type, int groupPos,
            ...
            pm.position = CyeeExpandableListPosition.obtain(type, groupPos, childPos, flatListPos);
     ...
     * Constructs the connector
     */
    public CyeeExpandableListConnector(Context context, ExpandableListAdapter expandableListAdapter) {
        mExpGroupMetadataList = new ArrayList<GroupMetadata>();
        ...
        // Gionee <gaoj> <2013-11-01> add for CR00833450 begin
        mChildItemHeight = (int) context.getResources().getDimension(
                com.cyee.internal.R.dimen.cyee_expandablelistview_child_item);
        // Gionee <gaoj> <2013-11-01> add for CR00833450 end
    }
    ...
    public int[] mHeight = { 13, 18, 28, 43, 63, 88, 118 };
    /*private final ViewGroup.LayoutParams mParams = new AbsListView.LayoutParams(
    ...
    private Animator createExpandAnimaForView(final View view, int pos) {
        ...
        Log.v("CyeeExpandListConnector", "mChildItemHeight = " + mChildItemHeight);
        if (ly == null) {
            ly = new AbsListView.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT);
        }
        final ViewGroup.LayoutParams lp = ly;
        mItemHeight = lp.height;

        Log.v("CyeeExpandListConnector", "open  pos = " + pos);
        int height = (pos < 7 ? mChildItemHeight - mHeight[pos] : 1);
        Log.v("CyeeExpandListConnector", "heght = " + height);
        final ValueAnimator animator = ValueAnimator.ofInt(height, mChildItemHeight);
        ...
        animator.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator arg0) {
                Log.v("CyeeExpandListConnector",
                        "createExpandAnimaForView onAnimationEnd");
                view.setAlpha(1.0f);
        ...
        animator.addUpdateListener(new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                Log.v("CyeeExpandListConnector",
                        "createExpandAnimaForView onAnimationUpdate lp.height = "
                                + lp.height);
                if (lp.height < (mChildItemHeight - 8)) {
                    view.setAlpha(0.0f);
                } else {
                    view.setAlpha(1.0f);
                }
    ...
    private void getFrameListBackground(Context context) {
        ...
        context.getTheme().resolveAttribute(com.cyee.internal.R.attr.cyeeframeListBackground,
                outValue, true);
        mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_FULL] = outValue.resourceId;
        context.getTheme().resolveAttribute(com.cyee.internal.R.attr.cyeeframeListTopBackground,
                outValue, true);
        mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_TOP] = outValue.resourceId;
        context.getTheme().resolveAttribute(
                com.cyee.internal.R.attr.cyeeframeListMiddleBackground, outValue, true);
        mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_MIDDLE] = outValue.resourceId;
        context.getTheme().resolveAttribute(
                com.cyee.internal.R.attr.cyeeframeListBottomBackground, outValue, true);
