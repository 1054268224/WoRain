...
class CyeeFastScroller {
    ...
    private final CyeeListView mList;
    ...
    public CyeeFastScroller(CyeeListView listView, int styleResId) {
        mList = listView;
	...
	private void changeColor() {
        if (ChameleonColorManager.isNeedChangeColor(mList.getContext())) {
            
            if (mThumbDrawable != null && mThumbDrawable instanceof StateListDrawable) {
                StateListDrawable stateListDrawble = (StateListDrawable) mThumbDrawable;
                for (int i = 0; i < stateListDrawble.getStateCount(); i++) {
                    Drawable drawable = stateListDrawble.getStateDrawable(i);
                    boolean pressed = stateIsPressed(stateListDrawble.getStateSet(i));
                    setThumbDrawableTint(drawable, pressed);
                }
            }
            
            
            if (mTrackDrawable != null && mTrackDrawable instanceof StateListDrawable) {
                StateListDrawable stateListDrawble = (StateListDrawable) mTrackDrawable;
                for (int i = 0; i < stateListDrawble.getStateCount(); i++) {
                    Drawable drawable = stateListDrawble.getStateDrawable(i);
                    boolean pressed = stateIsPressed(stateListDrawble.getStateSet(i));
                    setTrackDrawableTint(drawable, pressed);
                }
            }
        }
    ...
    private void setThumbDrawableTint(Drawable drawable, boolean pressed) {
        int color = pressed ? ChameleonColorManager.getAccentColor_G2() : ChameleonColorManager
                .getAccentColor_G1();
        drawable.setTint(color);
    }	
    
    private void setTrackDrawableTint(Drawable drawable, boolean pressed) {
        int color = pressed ? ChameleonColorManager.getAccentColor_G1() : ChameleonColorManager
                .getContentColorSecondaryOnBackgroud_C2();
        drawable.setTint(color);
    }
	
	private boolean stateIsPressed(int[] myDrawableState) {
        for(int i = 0; i < myDrawableState.length; i++){
            if(myDrawableState[i] ==  android.R.attr.state_pressed){
                return true;
            }
        }
        return false;
    ...
    private static int getFastScrollerStyleableField(String fieldName) {
        try {
            return com.android.internal.R.styleable.class.getField(fieldName).getInt(null);
        } catch (Exception e) {
            e.printStackTrace();
            return -1;
        } 
    ...
    private void getSectionsFromIndexer() {
        ...
        if (adapter instanceof CyeeExpandableListConnector) {
            final ExpandableListAdapter expAdapter = ((CyeeExpandableListConnector) adapter)
                    .getAdapter();
    ...
    private void scrollTo(float position) {
        ...
        if (sections != null && sectionCount > 1) {
            ...
            if (mList instanceof CyeeExpandableListView) {
                final CyeeExpandableListView expList = (CyeeExpandableListView) mList;
            ...
            if (mList instanceof CyeeExpandableListView) {
            	CyeeExpandableListView expList = (CyeeExpandableListView) mList;
                expList.setSelectionFromTop(expList.getFlatListPosition(
     ...
     */
    private static final Property<View, Integer> LEFT = new IntProperty<View>("left") {
        @Override
     ...
     */
    private static final Property<View, Integer> TOP = new IntProperty<View>("top") {
        @Override
        public void setValue(View object, int value) {
            object.setTop(value);
        }
     ...
     */
    private static final Property<View, Integer> RIGHT = new IntProperty<View>("right") {
        @Override
        public void setValue(View object, int value) {
     ...
     */
    private static final Property<View, Integer> BOTTOM = new IntProperty<View>("bottom") {
        @Override
        public void setValue(View object, int value) {
