...
abstract class CyeeGenericInflater<T, P extends CyeeGenericInflater.Parent> {
     ...
     *            retrieved.
     */
    protected CyeeGenericInflater(Context context) {
        mContext = context;
    }
     ...
     */
    protected CyeeGenericInflater(CyeeGenericInflater<T,P> original, Context newContext) {
        mContext = newContext;
     ...
     * the given Context.
     */
    public abstract CyeeGenericInflater cloneInContext(Context newContext);
    ...
    public final T createItem(String name, String prefix, AttributeSet attrs)
        ...
        try {
        	...
        	if (isNativePreferenceClass(name)) {
        		...
        		NativePreferenceManager.setAnalyzeNativePreferenceXml(true);
        		name = "cyee.preference.Cyee" + name.substring(name.lastIndexOf('.') + 1);
        	}else {
            ...
            InflateException ie = new InflateException(attrs
                    .getPositionDescription()
                    + ": Error inflating class "
                    + constructor.getClass().getName(), e);
            throw ie;
        }
    }
    ...
    private boolean isNativePreferenceClass(String name) {
        if (-1 != name.indexOf('.')) {
            return false;
        } else return name.equals("PreferenceScreen") || name.equals("PreferenceGroup") || name.equals("PreferenceCategory") ||
