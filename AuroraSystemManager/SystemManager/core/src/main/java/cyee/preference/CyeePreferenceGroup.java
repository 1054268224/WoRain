 ...
 */
public abstract class CyeePreferenceGroup extends CyeePreference implements CyeeGenericInflater.Parent<CyeePreference> {
    /**
     ...
     * to this directly.
     */
    private final List<CyeePreference> mPreferenceList;
    ...
    public CyeePreferenceGroup(Context context, AttributeSet attrs, int defStyle, int defStyleRes) {
        ...
        // Gionee <zhangxx> <2013-08-14> modify for CR00857086 end
            TypedArray a = context.obtainStyledAttributes(attrs,
                    com.cyee.internal.R.styleable.CyeePreferenceGroup, defStyle, 0);
            mOrderingAsAdded = a.getBoolean(com.cyee.internal.R.styleable.CyeePreferenceGroup_cyeeorderingFromXml,
                    mOrderingAsAdded);
            a.recycle();
        // Gionee <zhangxx> <2013-08-14> modify for CR00857086 begin
        }
    ...
    public CyeePreferenceGroup(Context context, AttributeSet attrs, int defStyle) {
        this(context, attrs, defStyle, 0);
    }

    public CyeePreferenceGroup(Context context, AttributeSet attrs) {
     ...
     */
    public void addItemFromInflater(CyeePreference preference) {
     ...
     */
    public CyeePreference getPreference(int index) {
     ...
     */
    public boolean addPreference(CyeePreference preference) {
        if (mPreferenceList.contains(preference)) {
            // Exists
            return true;
        }
        
        if (preference.getOrder() == CyeePreference.DEFAULT_ORDER) {
            if (mOrderingAsAdded) {
            ...
            if (preference instanceof CyeePreferenceGroup) {
                ...
                ((CyeePreferenceGroup)preference).setOrderingAsAdded(mOrderingAsAdded);
            }
        }
     ...
     */
    public boolean removePreference(CyeePreference preference) {
        final boolean returnValue = removePreferenceInt(preference);
        notifyHierarchyChanged();
        return returnValue;
    }

    private boolean removePreferenceInt(CyeePreference preference) {
        synchronized(this) {
    ...
    public void removeAll() {
        synchronized(this) {
            List<CyeePreference> preferenceList = mPreferenceList;
    ...
    public CyeePreference findPreference(CharSequence key) {
        ...
        for (int i = 0; i < preferenceCount; i++) {
            final CyeePreference preference = getPreference(i);
            ...
            if (preference instanceof CyeePreferenceGroup) {
                final CyeePreference returnedPreference = ((CyeePreferenceGroup)preference)
                        .findPreference(key);
    ...
    // Gionee <weidong><2017-5-10> modify for 134934 begin
    /*
     * @Override public void setEnabled(boolean enabled) {
     * super.setEnabled(enabled);
     * 
     * // Dispatch to all contained preferences final int preferenceCount =
     * getPreferenceCount(); for (int i = 0; i < preferenceCount; i++) {
     * getPreference(i).setEnabled(enabled); } }
     */
    // Gionee <weidong><2017-5-10> modify for 134934 end
