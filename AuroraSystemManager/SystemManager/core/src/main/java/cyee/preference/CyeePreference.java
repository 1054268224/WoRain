...
import cyee.preference.CyeePreferenceGroupAdapter.ItemType;
import cyee.preference.CyeePreferenceGroupAdapter.PosType;
import cyee.preference.CyeePreferenceGroupAdapter.PreferenceType;
...
import com.cyee.internal.util.CyeeCharSequences;

public class CyeePreference implements Comparable<CyeePreference>, OnDependencyChangeListener { 
    
    private static final String LOGTAG = "CyeePreference";    
    /**
     * Specify for {@link #setOrder(int)} if a specific order is not required.
     */
    public static final int DEFAULT_ORDER = Integer.MAX_VALUE;
    private static final int MAX_TEXT_WIDTH = 294; // dp
    private static final int TEXT_MARGIN = 8; // dp
    ...
    private int mLayoutResId = com.cyee.internal.R.layout.cyee_preference;
    private int mWidgetLayoutResId;
    private boolean mHasSpecifiedLayout = false;
    
    private OnPreferenceChangeInternalListener mListener;
    
    private List<CyeePreference> mDependents;
    ...
    private int mCategoryDividerColorVal = -1;
    ...
    public interface OnPreferenceChangeListener {
         ...
         */
        boolean onPreferenceChange(CyeePreference preference, Object newValue);
    }
    ...
    public interface OnPreferenceClickListener {
         ...
         */
        boolean onPreferenceClick(CyeePreference preference);
    }
    ...
    interface OnPreferenceChangeInternalListener {
         ...
         */
        void onPreferenceChange(CyeePreference preference);
         ...
         * @param preference This Preference.
         */
        void onPreferenceHierarchyChange(CyeePreference preference);
    }
     ...
     */
    public CyeePreference(Context context, AttributeSet attrs, int defStyle, int defStyleRes) {
        mContext = context;
        ...
        if (NativePreferenceManager.getAnalyzeNativePreferenceXml() && attrs != null) {
            ...
            AnalyzeCyeePreferenceAttributeSet(context, attrs, defStyle, defStyleRes);
        }
        // Gionee <zhangxx> <2013-08-14> modifyfor CR00857086 end

        if (!getClass().getName().startsWith("cyee.preference")) {
            // For subclasses not in this package, assume the worst and don't cache views
            mHasSpecifiedLayout = true;
        }

        mMaxTextWidth = dip2px(context, MAX_TEXT_WIDTH);
        mTextMargin = dip2px(context, TEXT_MARGIN);
    }
     ...
     * @see #Preference(Context, AttributeSet)
     */
    public CyeePreference(Context context, AttributeSet attrs, int defStyle) {
     ...
     */
    public CyeePreference(Context context, AttributeSet attrs) {
     ...
     */
    public CyeePreference(Context context) {
        this(context, null);
    }
     ...
     * {@link R.id#widget_frame} to be the parent of the specific widget
     * for this Preference. It should similarly contain
     * {@link R.id#title} and {@link R.id#summary}.
     * 
    ...
    // Gionee <weidong><2015-1-21> modify for CR01438181 begin
    public void setPreferenceType(PreferenceType type) {
        mPreferenceType = type;
    }
    // Gionee <weidong><2015-1-21> modify for CR01438181 end
    /**
    ...
    protected View onCreateView(ViewGroup parent) {
        ...
        if (mLayoutResId == 0) {
            mLayoutResId = com.cyee.internal.R.layout.cyee_preference;
        }
        ...
        if (widgetFrame != null) {
            ImageView imgView = (ImageView) layout.findViewById(com.cyee.internal.R.id.cyee_preference_arrow_id);
    ...
    protected void onBindView(View view) {
        ...
        if (titleView != null) {
            ...
            if (!TextUtils.isEmpty(title)) {
                ...
                titleLen = titleView.getPaint().measureText(title.toString());
            } else {
        ...
        if (summaryView != null) {
            ...
            if (!TextUtils.isEmpty(summary)) {
                ...
                summaryLen = summaryView.getPaint().measureText(
                        summary.toString());
            } else {
        ...
        final TextView resultView = (TextView) view.findViewById(com.cyee.internal.R.id.cyee_preference_result);
        float resultLen = 0;
        if (null != resultView) {
            final CharSequence result = getPreferenceResult();
            if (!TextUtils.isEmpty(result)) {
                resultView.setText(result);
                resultView.setVisibility(View.VISIBLE);
                resultLen = resultView.getPaint()
                        .measureText(result.toString());
            } else {
                resultView.setVisibility(View.GONE);
            }
        }

        float startTxtLen = Math.max(titleLen, summaryLen);

        if (resultLen != 0) {
            float displayRegion = mMaxTextWidth - mTextMargin;
            if (startTxtLen + resultLen > displayRegion) {
                if (resultLen > displayRegion / 2
                        && startTxtLen > displayRegion / 2) {
                    resultView.setWidth((int) displayRegion / 2);
                    summaryView.setWidth((int) displayRegion / 2);
                    titleView.setWidth((int) displayRegion / 2);
                } else if (resultLen > displayRegion / 2) {
                    resultView.setWidth((int) (displayRegion - startTxtLen));
                } else if (startTxtLen > displayRegion / 2) {
                    resultView.setWidth((int) resultLen);
                } else {
                }
            }
        }
        ...
        if (mPreferenceType.itemType == ItemType.CATEGORY_PREFERENCE) {
            View dividerView = view.findViewById(com.cyee.internal.R.id.cyee_category_divider);
            if (null == dividerView) {
                return;
            }
            if (mPreferenceType.posType == PosType.TOP_POS) {
                dividerView.setVisibility(View.GONE);
            } else {
                dividerView.setVisibility(View.VISIBLE);
            }
            if (ChameleonColorManager.isNeedChangeColor(getContext())) {
                dividerView.setBackgroundColor(ChameleonColorManager.getCustomCategoryDividerC1());
            } else if (mCategoryDividerColorVal != -1) {
                dividerView.setBackgroundColor(mCategoryDividerColorVal);
            }
            View titleLayout = view.findViewById(com.cyee.internal.R.id.cyee_category_title_layout);
            View bottomLine = view.findViewById(com.cyee.internal.R.id.cyee_category_bottom_line);
            if (null == titleLayout || null == bottomLine) {
                return ;
            }
            if (titleView.getVisibility() == View.GONE || titleView.getVisibility() == View.INVISIBLE) {
            ...
            View pdividerView = view
                    .findViewById(com.cyee.internal.R.id.cyee_preference_divider);

            View bottomLineView = view
                    .findViewById(com.cyee.internal.R.id.cyee_preference_bottom_line);

            if (mPreferenceType.posType == PosType.END_POS) {
                if (null != bottomLineView) {
                    bottomLineView.setVisibility(View.VISIBLE);
                }
                if (null != pdividerView) {
                    pdividerView.setVisibility(View.GONE);
                }
            } else {
                if (null != bottomLineView) {
                ...
                if (null != pdividerView) {
                    pdividerView.setVisibility(View.VISIBLE);
                }
            }
        }
     ...
     * {@link R.id#title} within the View created by
     ...
     * {@link R.id#icon} within the View created by
     * {@link #onCreateView(ViewGroup)}.
     * 
    ...
    public void setIcon(int iconResId) {
        ...
        //Chenyee hushengsong 2018-06-22 modify for monkey error begin
        setIcon(mContext.getDrawable(iconResId));
        //Chenyee hushengsong 2018-06-22 modify for monkey error end
    }
    ...
    public CharSequence getPreferenceResult() {
        return mPreferenceResult;
    }
    
    /**
     * Sets the select result
     * @param result
     */
    public void setPreferenceResult(CharSequence result) {
        if (result == null && mPreferenceResult != null || result != null
                && !result.equals(mPreferenceResult)) {
            mPreferenceResult = result;
            notifyChanged();
        }
    }
    ...
    /**
     * 设置Category粗的分割线线条色值
     * @param colorVal 颜色的色值
     */
    public void setCategoryDividerColor(int colorVal) {
        mCategoryDividerColorVal = colorVal;
    }
     ...
     */
    public void performClick(CyeePreferenceScreen preferenceScreen) {
        ...
        CyeePreferenceManager preferenceManager = getPreferenceManager();
        if (preferenceManager != null) {
            CyeePreferenceManager.OnPreferenceTreeClickListener listener = preferenceManager
     ...
     */
    public int compareTo(CyeePreference another) {
        if (mOrder != DEFAULT_ORDER
            ...
            // Do name comparison
            return CyeeCharSequences.compareToIgnoreCase(mTitle, another.mTitle);
        }
    }
     ...
     */
    protected void onAttachedToHierarchy(CyeePreferenceManager preferenceManager) {
    ...
    private void registerDependency() {
        ...
        CyeePreference preference = findPreferenceInHierarchy(mDependencyKey);
        if (preference != null) {
    ...
    private void unregisterDependency() {
        if (mDependencyKey != null) {
            final CyeePreference oldDependency = findPreferenceInHierarchy(mDependencyKey);
     ...
     */
    protected CyeePreference findPreferenceInHierarchy(String key) {
     ...
     */
    private void registerDependent(CyeePreference dependent) {
        if (mDependents == null) {
            mDependents = new ArrayList<CyeePreference>();
        }
     ...
     *         into a single statement.
     */
    private void unregisterDependent(CyeePreference dependent) {
        if (mDependents != null) {
    ...
    public void notifyDependencyChange(boolean disableDependents) {
        final List<CyeePreference> dependents = mDependents;
     ...
     */
    public void onDependencyChanged(CyeePreference dependency, boolean disableDependent) {
     ...
     */
    public void onParentChanged(CyeePreference parent, boolean disableChild) {
        ...
        for (i=0; i<attrs.getAttributeCount(); i++) {
            ...
            switch (id) {
                ...
                case android.R.attr.dependency:
                    mDependencyKey = NativePreferenceManager.getAttributeStringValue(context, attrs, i);
                    break;
    ...
    private void AnalyzeCyeePreferenceAttributeSet(Context context, AttributeSet attrs, int defStyle, int defStyleRes) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.CyeePreference, defStyle, defStyleRes);
        for (int i = a.getIndexCount(); i >= 0; i--) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.CyeePreference_cyeeicon) {
                mIconResId = a.getResourceId(attr, 0);
            } else if (attr == R.styleable.CyeePreference_cyeekey) {
                mKey = a.getString(attr);
            } else if (attr == R.styleable.CyeePreference_cyeetitle) {
                mTitleRes = a.getResourceId(attr, 0);
                mTitle = a.getString(attr);
            } else if (attr == R.styleable.CyeePreference_cyeesummary) {
                mSummary = a.getString(attr);
            } else if (attr == R.styleable.CyeePreference_cyeeorder) {
                mOrder = a.getInt(attr, mOrder);
            } else if (attr == R.styleable.CyeePreference_cyeefragment) {
                mFragment = a.getString(attr);
            } else if (attr == R.styleable.CyeePreference_cyeelayout) {
                mLayoutResId = a.getResourceId(attr, mLayoutResId);
            } else if (attr == R.styleable.CyeePreference_cyeewidgetLayout) {
                mWidgetLayoutResId = a.getResourceId(attr, mWidgetLayoutResId);
            } else if (attr == R.styleable.CyeePreference_cyeeenabled) {
                mEnabled = a.getBoolean(attr, true);
            } else if (attr == R.styleable.CyeePreference_cyeeselectable) {
                mSelectable = a.getBoolean(attr, true);
            } else if (attr == R.styleable.CyeePreference_cyeepersistent) {
                mPersistent = a.getBoolean(attr, mPersistent);
            } else if (attr == R.styleable.CyeePreference_cyeedependency) {
                mDependencyKey = a.getString(attr);
            } else if (attr == R.styleable.CyeePreference_cyeedefaultValue) {
                mDefaultValue = onGetDefaultValue(a, attr);
            } else if (attr == R.styleable.CyeePreference_cyeeshouldDisableView) {
                mShouldDisableView = a.getBoolean(attr, mShouldDisableView);
            } else if (attr == R.styleable.CyeePreference_cyeedefaultResult) {
                mPreferenceResult = a.getString(attr);
            } else if (attr == R.styleable.CyeePreference_cyeeShowPreferenceArrow) {
                mShowArrow = a.getBoolean(attr, mShowArrow);
            } else if (attr == R.styleable.CyeePreference_cyeeCategoryDividerColor) {
                mCategoryDividerColorVal = a.getColor(attr, mCategoryDividerColorVal);
            }
        }
        a.recycle();        
    }
